...
//<>, Басаргин (16.01.2017) {
Функция НайтиВопросАнкетыПоНаименованию()
КонецФункции
//<> }
...
Функция ДобавитьЭлементСправочникаВопросШаблонаАнкеты(СтрокаДерева,Код = Неопределено,СправочникРодитель = Неопределено,ВопросРодитель = Неопределено)
	...
	//<>, Басаргин (16.01.2017) {
	//to do: функция НайтиВопросАнкетыПоНаименованию()
	Если СтрокаДерева.ВопросШаблона.Пустая() Тогда		
		Если СтрокаДерева.ТипСтроки = "Раздел" Тогда
			ТекОбъект = Справочники.ВопросыШаблонаАнкеты.СоздатьГруппу();
		Иначе			
			ТекОбъект = Справочники.ВопросыШаблонаАнкеты.СоздатьЭлемент();
			ТекОбъект.ТипВопроса                        = СтрокаДерева.ТипВопроса;
			ТекОбъект.ЭлементарныйВопрос                = СтрокаДерева.ЭлементарныйВопрос;
			ТекОбъект.ТипТабличногоВопроса              = СтрокаДерева.ТипТабличногоВопроса;
			ТекОбъект.Обязательный                      = СтрокаДерева.Обязательный;
			ТекОбъект.РодительВопрос                    = ?(ВопросРодитель = Неопределено, Справочники.ВопросыШаблонаАнкеты.ПустаяСсылка(),ВопросРодитель);
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СтрокаДерева.СоставТабличногоВопроса,ТекОбъект.СоставТабличногоВопроса);
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(СтрокаДерева.ПредопределенныеОтветы,ТекОбъект.ПредопределенныеОтветы);			
		КонецЕсли;		
	Иначе
		ТекОбъект = СтрокаДерева.ВопросШаблона.ПолучитьОбъект();
	КонецЕсли;	
	//<> }
	
	Если СтрокаДерева.ТипСтроки = "Раздел" Тогда
		
		СпрОбъект = ТекОбъект;//Справочники.ВопросыШаблонаАнкеты.СоздатьГруппу();
		
	Иначе
		
		СпрОбъект = ТекОбъект;//Справочники.ВопросыШаблонаАнкеты.СоздатьЭлемент();
		
		//<>, Басаргин (01.02.2017) {
		СпрОбъект.СоставТабличногоВопроса.Очистить();
		СпрОбъект.ПредопределенныеОтветы.Очистить();  //СпрОбъект.ТипВопроса = Перечисления.ТипыВопросовШаблонаАнкеты.Табличный
		//<> }
