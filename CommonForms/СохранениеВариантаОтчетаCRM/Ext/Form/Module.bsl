...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	////<>, Басаргин (25.12.2017) {	
	//Если НЕ Контекст.ЭтоПолныеПрава Тогда
	//	Доступен = "1";
	...
	//<747854>, Басаргин (09.01.2018) {	
	УстановитьДоступностьВариантаДляАвтора();
	//<747854> }	
...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция = CRMСервер.ИнструкцияУсловногоОформления();
	...
	CRMСервер.ДобавитьЭлементУсловногоОформления(ЭтотОбъект, Инструкция);
...
Процедура ЗаполнитьСписокВариантов()
	...
	Если Контекст.ЭтоВнешний Тогда
		...
		Исключение
			ВариантыОтчетов.ЗаписатьВЖурнал(Неопределено,
				СтрШаблон(
...
Процедура ПрочитатьДанныеВарианта()
	...
	Если ТипЗнч(ВариантСсылка)=Тип("СправочникСсылка.ВариантыДополнительныхОтчетов") Тогда
		...
		|	ЕСТЬNULL(НастройкиВариантовОтчетовCRM.ФункциональнаяОпция, """") КАК ФункциональнаяОпция,
		|	ЕСТЬNULL(НастройкиВариантовОтчетовCRM.Образец, НЕОПРЕДЕЛЕНО) КАК Образец,
		|	ЕСТЬNULL(НастройкиВариантовОтчетовCRM.Теги, """") КАК Теги,
		|	ЕСТЬNULL(НастройкиВариантовОтчетовCRM.Рекомендуемый, ЛОЖЬ) КАК Рекомендуемый
		|ИЗ
		...
		|		ПО ВариантыДополнительныхОтчетов.Ссылка = НастройкиВариантовОтчетовCRM.Вариант
		|ГДЕ
	...
	ИначеЕсли ТипЗнч(ВариантСсылка)=Тип("СправочникСсылка.ВариантыОтчетов") Тогда 
		...
		|	Справочник.ВариантыОтчетов КАК ВариантыОтчетов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетовCRM КАК НастройкиВариантовОтчетовCRM
		|		ПО ВариантыОтчетов.Ссылка = НастройкиВариантовОтчетовCRM.Вариант
		|ГДЕ
...
Процедура СохранитьНастройкиВарианта()
	...
	// Настройки вариантов CRM
	Менеджер = РегистрыСведений.НастройкиВариантовОтчетовCRM.СоздатьМенеджерЗаписи();
...
Процедура ПрочитатьДанныеПривязок()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПривязкаПолейОтчетовCRM.Поле,
	|	ПривязкаПолейОтчетовCRM.Объект,
	|	ПривязкаПолейОтчетовCRM.СложныйАлгоритм,
	|	ПривязкаПолейОтчетовCRM.ДополнительныйРазрез,
	|	ПривязкаПолейОтчетовCRM.Объект.Синоним КАК Представление
	|ИЗ
	|	РегистрСведений.ПривязкаПолейОтчетовCRM КАК ПривязкаПолейОтчетовCRM
	|ГДЕ
	|	ПривязкаПолейОтчетовCRM.Вариант = &Вариант";
...
//<747854>, Басаргин (09.01.2018) {
&НаСервере
Процедура УстановитьДоступностьВариантаДляАвтора()
	
	Если НЕ ТипЗнч(Контекст) = Тип("Структура") ИЛИ  НЕ Контекст.Свойство("ЭтоПолныеПрава") Тогда
		Возврат;
	КонецЕсли;
