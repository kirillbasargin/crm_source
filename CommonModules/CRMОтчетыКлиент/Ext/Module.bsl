...
Процедура ОбработкаРасшифровки(ЭтаФорма, Элемент, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ЭтаФорма.НастройкиОтчета.ПолноеИмя = "Отчет.Квартирограмма"
		...
		Если ЗначениеЗаполнено(ЗначениеОН) Тогда
			...
			//НоваяСтрока.Площадь = CRMОтчетыВызовСервера.ПолучитьЗначениеРасшифровкиБезАнализа("Площадь", Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);
			...
			НоваяСтрока.НомерКвартиры = ?(ЗначениеЗаполнено(НомерКвартирыБТИ), НомерКвартирыБТИ, НомерКвартиры);
			...
			НоваяСтрока.Площадь = ?(ЗначениеЗаполнено(ПлощадьБТИ), ПлощадьБТИ, Площадь);
