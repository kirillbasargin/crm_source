...
//<>, Басаргин (24.10.2017) {
Процедура Взаимодействия_ЗаписатьВФоне(ТЗ_Взаимодействия) Экспорт
	...
	Для каждого Строка Из ТЗ_Взаимодействия Цикл
		Если НЕ Строка.Записывать Тогда
			Продолжить;
		КонецЕсли;
		Попытка
			РегистрыСведений.Взаимодействия.ЗаписатьВзаимодействие(Строка.Взаимодействие.ПолучитьОбъект());
		Исключение
			Продолжить;
		КонецПопытки;
	КонецЦикла;
	...
	Если ОценкаПроизводительностиСуществует Тогда
		МодульОценкаПроизводительности.ЗакончитьЗамерВремени("Взаимодействия_ЗаписатьВФоне", ВремяНачала);
	КонецЕсли;		
	
КонецПроцедуры
//<> }

//<770817>, Басаргин (29.01.2018) {
Процедура ДозаполнениеСоответствияЗапросовЗвонкам() Экспорт
	...
	|	ВЫРАЗИТЬ(СоответствиеЗапросовЗвонкам.Взаимодействие.АбонентКонтакт КАК Справочник.Клиенты) КАК Клиент
	|ПОМЕСТИТЬ ВТ_Соответствие
	|ИЗ
	|	РегистрСведений.СоответствиеЗапросовЗвонкам КАК СоответствиеЗапросовЗвонкам
	|ГДЕ
	|	СоответствиеЗапросовЗвонкам.ДатаСвязи МЕЖДУ &ПериодНачало И &ПериодКонец
	|	И СоответствиеЗапросовЗвонкам.Взаимодействие <> ЗНАЧЕНИЕ(Документ.ТелефонныйЗвонок.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Взаимодействие,
	|	ДатаСвязи
	...
	|ГДЕ
	|	ВЫРАЗИТЬ(Взаимодействия.Контакт КАК Справочник.Клиенты) В
	|			(ВЫБРАТЬ
	|				ВТ_Соответствие.Клиент
	|			ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Взаимодействия.ДатаВзаимодействия КАК ДатаСвязи,
	|	ВТ_Соответствие.ID_Звонка КАК ID_Звонка,
	|	ВТ_Взаимодействия.Взаимодействие КАК Взаимодействие,
	|	ВТ_Взаимодействия.Предмет КАК Запрос
	|ИЗ
	|	ВТ_Соответствие КАК ВТ_Соответствие
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Взаимодействия КАК ВТ_Взаимодействия
	|		ПО ВТ_Соответствие.Клиент = ВТ_Взаимодействия.Клиент
	|			И (НАЧАЛОПЕРИОДА(ВТ_Соответствие.ДатаСвязи, ДЕНЬ) = НАЧАЛОПЕРИОДА(ВТ_Взаимодействия.ДатаВзаимодействия, ДЕНЬ))";
	
	Запрос.УстановитьПараметр("ПериодКонец", КонецДня(ТекущаяДата()) - 86400);
	Запрос.УстановитьПараметр("ПериодНачало", НачалоДня(ТекущаяДата()) - 86400);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НаборЗаписей.Отбор.Запрос.Установить(ВыборкаДетальныеЗаписи.Запрос);
		...
		ЗаписьНабора.ID_Звонка = ВыборкаДетальныеЗаписи.ID_Звонка;
		ЗаписьНабора.Запрос	= ВыборкаДетальныеЗаписи.Запрос;
		ЗаписьНабора.ДатаСвязи = ВыборкаДетальныеЗаписи.ДатаСвязи;
		ЗаписьНабора.Взаимодействие = ВыборкаДетальныеЗаписи.Взаимодействие;
...
Функция ПараметрыПисьмаДляОтправки(Объект)
	...
	Пока ВыборкаВложения.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
			...
			//EXTCODE Шумилин Сергей 24.08.2017 {{ ---> Перенёс снизу под условие, так как подсистемы "ЭлектроннаяПодпись" у нас нет
			НомерСтроки = 1;
			...
			КонецЦикла;
			//EXTCODE Шумилин Сергей 24.08.2017 <--- }} 
...
Процедура ОтправкаSMS() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |					И СообщениеSMS.ДатаКогдаОтправить < &ТекущаяДата
	               |				ТОГДА ИСТИНА
...
Процедура ОбновлениеСтатусовДоставкиSMS() Экспорт
	...
	//++ Юкаев Роман 20180207 ( //773137
	|	И РАЗНОСТЬДАТ(СообщениеSMSАдресаты.Ссылка.Дата, &Дата, ДЕНЬ) <= 2
	//-- Юкаев Роман 20180207 )
	|ИТОГИ ПО
	|	Ссылка";
...
Процедура ОтразитьРезультатыОтправкиSMSВДокументе(ДокументОбъект, РезультатОтправки)
	...
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		...
		КонецЦикла;
	КонецЦикла;
	//EXTCODE Шумилин Сергей 31.01.2018 {{ --->
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		//++ Юкаев Роман 20180326 (//
		НомерДляОтправки = СтрЗаменить(ОтправленноеСообщение.НомерПолучателя, "+", "");
		
		ПродолжитьПроверку = Истина;
		
		Если СтрДлина(НомерДляОтправки) = 11 Тогда 
			Если СтрНачинаетсяС(НомерДляОтправки, "7") Тогда //Российские номера
				
				Номер = "8" + Сред(НомерДляОтправки, 2);
				СписокСтрок = ДокументОбъект.Адресаты.НайтиСтроки(Новый Структура("НомерДляОтправки", Номер));
		...
		Если ПродолжитьПроверку Тогда
			СписокСтрок = ДокументОбъект.Адресаты.НайтиСтроки(Новый Структура("НомерДляОтправки", НомерДляОтправки));
	...
	//++ Юкаев Роман 20180326 (//
	Для Каждого СтрТЧ Из ДокументОбъект.Адресаты Цикл
		Если СтрТЧ.СостояниеСообщения = Перечисления.СостоянияСообщенияSMS.Исходящее Или СтрТЧ.ИдентификаторСообщения = "" Тогда
			СтрТЧ.СостояниеСообщения = Перечисления.СостоянияСообщенияSMS.ОшибкаПолученияСтатусаУПровайдера;
		КонецЕсли;
	КонецЦикла;
	//-- Юкаев Роман 20180326 )
