...
Процедура СоздатьНовоеВзаимодействие(ТипОбъекта,ПараметрыСоздания = Неопределено, Форма = Неопределено) Экспорт
	...
	//++ Юкаев Роман 20171214 ( //Задача 750854
	//ОткрытьФорму("Документ."+ ТипОбъекта+".ФормаОбъекта",ПараметрыСоздания, Форма);
	
	Если ПараметрыСоздания.Свойство("ЗначенияЗаполнения") И ПараметрыСоздания.ЗначенияЗаполнения.Предмет <> Неопределено Тогда
		СписокДокументов = Новый Массив();
		СписокДокументов.Добавить(ПараметрыСоздания.ЗначенияЗаполнения.Предмет);
		МассивНепроведенных = ОбщегоНазначенияВызовСервера.ПроверитьПроведенностьДокументов(СписокДокументов);
		
		Если МассивНепроведенных.Количество() > 0 Тогда
			
			ТекстСообщения = НСтр("ru = 'Перед созданием взаимодействия необходимо провести запрос. Провести запрос?'");
			
			ДопПараметры = Новый Структура("Форма,ПараметрыСоздания,ТипОбъекта,СписокДокументов", Форма, ПараметрыСоздания, ТипОбъекта, СписокДокументов);
			
			Оповещение = Новый ОписаниеОповещения("ЗапросНаПроведениеЗавершение", ЭтотОбъект, ДопПараметры);
...
//++ Юкаев Роман 20171214 ( //
Процедура ЗапросНаПроведениеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		
		ОбщегоНазначенияВызовСервера.ПровестиДокументы(ДополнительныеПараметры.СписокДокументов);
		ОткрытьФорму("Документ." + ДополнительныеПараметры.ТипОбъекта + ".ФормаОбъекта", ДополнительныеПараметры.ПараметрыСоздания, ДополнительныеПараметры.Форма);
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Взаимодействие не создано т.к. не проведен запрос!";
		Сообщение.Сообщить();
	КонецЕсли;
