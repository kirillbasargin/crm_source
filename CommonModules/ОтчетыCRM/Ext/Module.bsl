...
Функция ОбразецВарианта(Вариант) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиВариантовОтчетовCRM.Образец
	|ИЗ
	|	РегистрСведений.НастройкиВариантовОтчетовCRM КАК НастройкиВариантовОтчетовCRM
	|ГДЕ
	|	НастройкиВариантовОтчетовCRM.Вариант = &Вариант";
...
Процедура ДобавитьВариантыОформленияСумм(ВариантыОформления, Поля, НастройкиКД = Неопределено) Экспорт
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		...
		ОтчетыCRM.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=2");
		
		КлючВариантаОформления = НСтр("ru = 'Без копеек'");
		ОтчетыCRM.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0");
		
		КлючВариантаОформления = НСтр("ru = 'В тысячах'");
		ОтчетыCRM.ДобавитьВариантОформления(ВариантыОформления, ИмяПоля, КлючВариантаОформления, "Формат", "ЧДЦ=0; ЧС=3");
			
	КонецЦикла; 
	
	Если НЕ НастройкиКД=Неопределено Тогда
		Для каждого ИмяПоля Из МассивПолей Цикл
			ОтчетыCRMКлиентСервер.ДобавитьУсловноеОформление(НастройкиКД, ИмяПоля, ВариантыОформления[ИмяПоля][КлючОсновногоВариантаОформления]); 
		КонецЦикла; 
	КонецЕсли; 
...
Процедура ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных) Экспорт
	...
	Если НЕ Поле=Неопределено Тогда
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "ОтчетыCRM.МесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "ОтчетыCRM.ДеньМесяцГода("+ИмяПоля+")";
		...
		Если ВычисляемоеПоле=Неопределено Тогда
			...
			ВычисляемоеПоле.Выражение = "ОтчетыCRM.НомерНеделиГода("+ИмяПоля+")";
...
Процедура ПриКомпоновкеРезультата(КомпоновщикНастроек, СхемаКомпоновкиДанных, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка) Экспорт
	...
	CRMОтчеты.УстановитьМакетОформленияОтчета(НастройкиОтчета);
	CRMОтчеты.ВывестиЗаголовокОтчета(ПараметрыОтчета, ДокументРезультат);
	...
	Если ДополнительныеСвойства.Свойство("ИспользоватьСравнение") И ДополнительныеСвойства.ИспользоватьСравнение Тогда
		CRMОтчеты.НастроитьДинамическийПериод(СхемаКомпоновкиДанных, ПараметрыОтчета);
...
Функция ТаблицаПериодов(НачалоПериода, КонецПериода, ПараметрыОтчета) Экспорт
	...
	Иначе
		Периодичность = CRMОтчеты.ПолучитьЗначениеПериодичности(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
	КонецЕсли;
...
// в регистрах сведений НастройкиВариантовОтчетовCRM и ПривязкаПолейОтчетовCRM
//
// Параметры:
...
//
// Места использования:
//   ОбщаяФорма.ОтчетыCRM.ВнестиИзмененияВРеквизитыНаФорме()
//
...
Процедура ПодпискаНаСобытиеДополнительныйОтчетПриЗаписи(Источник, Отказ) Экспорт
	...
	Если НЕ Источник.ЭтоНовый() Тогда
		...
		КонецЦикла; 
	КонецЕсли; 
	Если НЕ МетаданныеОбъекта.Реквизиты.Найти("ЭтоОтчетCRM")=Неопределено И ОтчетОбъект.ЭтоОтчетCRM Тогда
...
Процедура ПриСозданииНаСервереФормыСвязанногоОбъекта(Форма) Экспорт
	...
	ИдентификаторОбъектаМетаданных = ОтчетыCRMПовтИсп.ИдентификаторОбъектаМетаданныхПоИмениФормы(Форма.ИмяФормы);
...
Функция РезультатыЗапросаВариантыОтчетов(МассивИдентификаторов = Неопределено)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НастройкиВариантовОтчетовCRM.Вариант
	|ИЗ
	|	РегистрСведений.НастройкиВариантовОтчетовCRM КАК НастройкиВариантовОтчетовCRM
	|ГДЕ
	|	НЕ НастройкиВариантовОтчетовCRM.Вариант В
	|				(ВЫБРАТЬ
	|					АктуальныеВарианты.Вариант
	|				ИЗ
	|					АктуальныеВарианты КАК АктуальныеВарианты)
	|	И (&БезОтбора
	|			ИЛИ НастройкиВариантовОтчетовCRM.Вариант.Отчет В (&Отчеты))
	|
	|СГРУППИРОВАТЬ ПО
	|	НастройкиВариантовОтчетовCRM.Вариант
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПривязкаПолейОтчетовCRM.Вариант
	|ИЗ
	|	РегистрСведений.ПривязкаПолейОтчетовCRM КАК ПривязкаПолейОтчетовCRM
	|ГДЕ
	|	НЕ ПривязкаПолейОтчетовCRM.Вариант В
	|				(ВЫБРАТЬ
	|					АктуальныеВарианты.Вариант
	|				ИЗ
	|					АктуальныеВарианты КАК АктуальныеВарианты)
	|	И (&БезОтбора
	|			ИЛИ ПривязкаПолейОтчетовCRM.Вариант.Отчет В (&Отчеты))
	|
	|СГРУППИРОВАТЬ ПО
	|	ПривязкаПолейОтчетовCRM.Вариант
	|;
...
Процедура УдалитьНеактуальныеНастройки(Результаты)
	...
	Пока ВыборкаНастройки.Следующий() Цикл
		Набор = РегистрыСведений.НастройкиВариантовОтчетовCRM.СоздатьНаборЗаписей();
	...
	Пока ВыборкаПоля.Следующий() Цикл
		Набор = РегистрыСведений.ПривязкаПолейОтчетовCRM.СоздатьНаборЗаписей();
		Набор.Отбор.Вариант.Установить(ВыборкаНастройки.Вариант);
		Набор.Записать(Истина);
	КонецЦикла; 
...
Процедура ОбновитьНастройкиОтчетов(Результаты)
	...
	Пока ВыборкаОтчеты.Следующий() Цикл
		...
		Если НЕ МетаданныеОбъекта=Неопределено И НЕ МетаданныеОбъекта.Реквизиты.Найти("ЭтоОтчетCRM")=Неопределено И ОтчетОбъект.ЭтоОтчетCRM Тогда
			Попытка
				ОтчетОбъект.ПриОпределенииНастроекОтчета(НастройкиОтчета, НастройкиВариантов);
			Исключение
				ТекстСообщения = СтрШаблон(
...
Процедура ОбновитьНастройкиВарианта(Вариант, СтруктураНастроек, ЗаменятьСуществующие = Ложь)
	...
	Запись = РегистрыСведений.НастройкиВариантовОтчетовCRM.СоздатьМенеджерЗаписи();
	...
	Если СтруктураНастроек.Свойство("СвязанныеПоля") И (ОбновитьПривязки ИЛИ ЗаменятьСуществующие) Тогда
		Для каждого ОписаниеПоля Из СтруктураНастроек.СвязанныеПоля Цикл
			...
			Исключение
				...
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.РегистрыСведений.ПривязкаПолейОтчетовCRM,,
				ТекстСообщения);
			...
			Если ЗначениеЗаполнено(ОписаниеПоля.ДополнительныйРазрез) Тогда
				...
				Запись = РегистрыСведений.ПривязкаПолейОтчетовCRM.СоздатьМенеджерЗаписи();
				Запись.Вариант = Вариант;
...
Процедура АктулизироватьТеги(МассивИдентификаторов = Неопределено)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиВариантовОтчетовCRM.Теги
	|ИЗ
	|	РегистрСведений.НастройкиВариантовОтчетовCRM КАК НастройкиВариантовОтчетовCRM
	|ГДЕ
	|	(&БезОтбора
...
Функция СоздатьГруппу(Стр, Отображение = Неопределено, ЭтаФорма, Элементы) Экспорт
	...
	Группа.Заголовок = Новый ФорматированнаяСтрока(Стр.Заголовок);
	Группа.Подсказка = Новый ФорматированнаяСтрока(Стр.Заголовок);
...
Процедура ДобавитьСимволВалютыКЗаголовкамПолей(СхемаКомпоновкиДанных, Поля, Валюта = Неопределено) Экспорт
	...
	Если Валюта=Неопределено Тогда
		СимволВалюты = CRMПовтИсп.ПолучитьСимвольноеПредставлениеВалюты(Константы.ВалютаУчета.Получить());
	Иначе
		СимволВалюты = CRMПовтИсп.ПолучитьСимвольноеПредставлениеВалюты(Валюта);
	КонецЕсли;
