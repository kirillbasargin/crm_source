...
Процедура ПозвонитьПоТелефону(НомерТелефона) Экспорт
	...
	//<>, Басаргин (03.10.2017) {
	Попытка		
		Если ЗначениеЗаполнено(НомерТелефона) И глТелефония["Активирована"] Тогда
			УправлениеТелефониейКлиент.СделатьЗвонок(УправлениеТелефониейКлиентСервер.АнализВызываемогоНомера(УправлениеТелефониейКлиентСервер.ТолькоДопустимыеСимволыНомера(НомерТелефона)));
