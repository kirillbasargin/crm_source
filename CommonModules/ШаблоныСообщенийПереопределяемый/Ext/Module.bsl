...
Процедура ПриЗаполненииТелефоновПолучателейВСообщении(ПолучателиSMS, НазначениеШаблона, ПредметСообщения) Экспорт
	...
	Для Каждого ЭлементСоответствия ИЗ ПредметСообщения.ПроизвольныеПараметры Цикл
		Если ЭлементСоответствия.Ключ = "ПредметСтрокой"
				И НЕ ПустаяСтрока(ЭлементСоответствия.Значение) Тогда
			НоваяСтрока = ПолучателиSMS.Добавить();
			НоваяСтрока.НомерТелефона = ЭлементСоответствия.Значение;
		    НоваяСтрока.Представление = "<Не указан>";
		КонецЕсли;
	КонецЦикла;
	//EXTCODE Шумилин Сергей 03.11.2017 <--- }} 
