...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ОбработкаОбъект 							= ДанныеФормыВЗначение(Параметры.Объект, Тип("ОбработкаОбъект.РассылкаКлиентамСМС"));
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	МассивЗапроса = "";
	Попытка
		ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, РезультатЗапроса, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, Текст, ТекстСообщения, Тестирование, МассивЗапроса);
	Исключение
...
//
&НаСервере
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения, Тест, МассивЗапроса)
	...
	Результат = ОбъектОбработки().ВыполнитьЗапрос(Текст, МассивПараметров, ТабличныйДокументРезультата, ПараметрыВыводаЗапроса, ОтчетПоВыполнениюЗапроса, МеткаЗапроса, Тест);
	...
	Если Тест Тогда
		...
		Если ТипЗнч(Результат) = Тип("ТаблицаЗначений") Тогда
			...
			ОП = Новый ОписаниеТипов(МассивТипов);
			
			Для Каждого СТКолонка Из Результат.Колонки Цикл
				Если СТКолонка.ТипЗначения = ОП Или СТКолонка.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.Клиенты") Тогда
					Колонка = СТКолонка.Имя;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если Колонка = "" Тогда
				Сообщить("Не нейдена колонка с типом Справочник.Клиенты.");
				Сообщить("Возможно в колонке с клиентом составной тип, исползуйте функцию ""ВЫРАЗИТЬ(<колонка> КАК Справочник.Клиенты)"".");
			Иначе
				МассивЗапроса = Результат.ВыгрузитьКолонку(Колонка);
			КонецЕсли;
		КонецЕсли;
