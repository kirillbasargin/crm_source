...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Пока Выборка.Следующий() Цикл
		Если СписокИпотечныхБанков.НайтиПоЗначению(Выборка.Ссылка) = Неопределено И ЗначениеЗаполнено(Выборка.Ссылка.ИмяМакета) Тогда
...
Процедура ПостроениеФормыСогласноРаздела()
	...
	Анкетирование.ПостроениеФормыЗаполненияПоРазделу(ЭтотОбъект,ТекущиеДанныеДеревоРазделов, Истина);
...
Функция ПолучитьПараметрыПечати(ИпотечнаяАнкета, ИпотечныйБанк = Неопределено)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Конецесли;
			Если НЕ ПроверкаУсловияПрефиксСуффикс(ИпотечныйБанк, Выборка.ЭлементарныйВопрос, Выборка.Ответ, СтруктураЗаполнения.Префикс, СтруктураЗаполнения.Суффикс) Тогда
				СтруктураЗаполнения.Префикс = "";
				СтруктураЗаполнения.Суффикс = "";
			КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьМакет(ИмяФайла, Расширение, ПараметрыПечати)
	...
	Если Найти(ВРег(Расширение), "DOC") Тогда
		Попытка		
			...
			Для каждого ПараметрПечати Из ПараметрыПечати Цикл		
				СтрокаПоиска = ПараметрПечати.ПараметрПечати;
				СтрокаЗамены = "";
				Если НЕ ПараметрПечати.ИспользоватьСпецСимволДляЗамены Тогда
					СтрокаЗамены = ПараметрПечати.Префикс + ?(ТипЗнч(ПараметрПечати.Ответ) = Тип("Дата"), Строка(Формат(ПараметрПечати.Ответ, "ДФ=dd.MM.yyyy")), Строка(ПараметрПечати.Ответ)) + ПараметрПечати.Суффикс;	
				Иначе
					СтрокаЗамены = Символ(HexToDec("2611")); //2612 - ☒
				КонецЕсли;
				Doc.Content.Find.Execute(СтрокаПоиска, Ложь, Истина, Ложь, , , Истина, , Ложь, СтрокаЗамены, 00000002);
			КонецЦикла;				
        ...
        Для каждого Элемент из Список Цикл
			Если Элемент.Пометка Тогда
				...
				Если ЗначениеЗаполнено(ИмяФайлаШаблона) Тогда
					ЗаполнитьМакет(ИмяФайлаШаблона, Расширение, ПолучитьПараметрыПечати(Объект.Ссылка, Элемент.Значение)); 
				КонецЕсли;
            КонецЕсли;
        КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоРазделовПриАктивизацииПоля(Элемент)
...
&НаКлиенте
Процедура ПерерйтиКРазделу(Команда)
