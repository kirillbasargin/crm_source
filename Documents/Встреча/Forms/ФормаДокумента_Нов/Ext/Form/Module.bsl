...
Функция ВернутьПараметр(Предмет)
	Возврат Новый Структура("Статус, РасшифровкаСтатуса", Предмет.Статус, Предмет.РасшифровкаСтатуса);
КонецФункции
...
Процедура ПредметОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	//++
	Если ЗначениеЗаполнено(Предмет) Тогда
		ПарамПредмета = ВернутьПараметр(Предмет);
		Объект.Статус = ПарамПредмета.Статус;
		Объект.РасшифровкаСтатуса = ПарамПредмета.РасшифровкаСтатуса;
	КонецЕсли;
	//--
КонецПроцедуры
