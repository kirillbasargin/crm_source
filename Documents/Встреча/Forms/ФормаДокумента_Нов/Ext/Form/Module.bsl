...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		////++ Юкаев Роман 20180827 (//
		//Если ЗначениеЗаполнено(Предмет) Тогда
		//	ЗаполнитьЗначенияСвойств(Объект, Предмет, "Статус, РасшифровкаСтатуса");
		//КонецЕсли;
		////-- Юкаев Роман 20180827 )
	КонецЕсли;
	...
	Если Параметры.Ключ.Пустая() Тогда
		Если Параметры.Свойство("ДанныеУчастника") Тогда
			Если ТипЗнч(Параметры.ДанныеУчастника) = Тип("Массив") Тогда
				Для каждого ДанныеУчастника Из Параметры.ДанныеУчастника Цикл
		...
		КонецЕсли;
		//Если Параметры.Свойство("ГруппаОтветственного") Тогда
		//	Объект.ГруппаОтветственного = Параметры.ГруппаОтветственного;
		//КонецЕсли;
		Если Параметры.Свойство("Офис") Тогда
			Объект.МестоПроведенияВстречи = Параметры.Офис;
		КонецЕсли;
	КонецЕсли;
	//<828132> }
	...
	//<886114>, Басаргин (19.10.2018) {
	Если ЭтоМенеджерГПТ Тогда
		//Элементы.Описание.Доступность = НЕ Рассмотрено;
		Элементы.Тема.ЦветФона = ?(НЕ Рассмотрено И НЕ Объект.Отменено, WebЦвета.Персиковый, Новый Цвет);
	КонецЕсли;
	//<886114> }	
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	////<886114>, Басаргин (19.10.2018) {
	//Если ЭтоМенеджерГПТ Тогда
	//	Если НЕ Рассмотрено И НЕ Объект.Отменено И НЕ ЗначениеЗаполнено(Объект.Описание) Тогда			
	//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не заполнено описание взаимодействия", , "Описание", "Объект", Отказ);	
	//	КонецЕсли;
	//КонецЕсли;
	////<886114> }
...
Процедура МестоПроведенияВстречиПриИзменении(Элемент)
	...
	Объект.ГруппаОтветственного = Неопределено;
...
Процедура ЗаполнитьНаОснованииУчастника(ДанныеУчастника)
	...
	АбонентКонтакт = ДанныеУчастника.Контакт;
	АбонентПредставление = "";
	АбонентКакСвязаться = "";
	Если ПустаяСтрока(ДанныеУчастника.КакСвязаться) Тогда
		
		АбонентКакСвязаться = "";
		Взаимодействия.ДозаполнитьПоляКонтактов(АбонентКонтакт,
			АбонентПредставление,
			АбонентКакСвязаться,
	...
	Иначе
		...
		АбонентКакСвязаться = ДанныеУчастника.КакСвязаться;
		
	КонецЕсли;
	
	АбонентПредставление = ДанныеУчастника.Представление;
	
	НайденныеСтроки = Объект.Участники.НайтиСтроки(Новый Структура("Контакт", АбонентКонтакт));
	Если НЕ НайденныеСтроки.Количество() Тогда
		НоваяСтрока = Объект.Участники.Добавить();
		НоваяСтрока.Контакт = АбонентКонтакт;
		НоваяСтрока.ПредставлениеКонтакта = АбонентПредставление;
		НоваяСтрока.КакСвязаться = АбонентКакСвязаться;
	КонецЕсли;	
