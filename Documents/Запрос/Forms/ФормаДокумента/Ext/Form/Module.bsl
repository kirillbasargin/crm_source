...
Процедура СоздатьНаОсновании_ВыдачаДисконтныхКарт(Команда)
	...
	Если Объект.Участники.Количество() > 1 Тогда
		...
		КонецЦикла;		
		ОткрытьФорму("Документ.ВыдачаДисконтныйКарт.Форма.ФормаВыбораВыгодоприобретателя", Новый Структура("Запрос, Клиенты", Объект.Ссылка, Клиенты), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОткрытьФорму("Документ.ВыдачаДисконтныйКарт.ФормаОбъекта", Новый Структура("Основание", Объект.Ссылка), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);		
	КонецЕсли;
