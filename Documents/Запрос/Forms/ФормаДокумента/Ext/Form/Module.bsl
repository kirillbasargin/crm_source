...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Пользователи.РолиДоступны("МенеджерФилиала") И НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		КонецЕсли;
		////<899444>, Басаргин (19.11.2018) {
		//Если НЕ Объект.КоличествоКомнат_1 И НЕ Объект.КоличествоКомнат_2 И НЕ Объект.КоличествоКомнат_3 И НЕ Объект.КоличествоКомнат_4 Тогда
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбрано количество комнат", Объект.Ссылка, "КоличествоКомнат_1", "Объект", Отказ);			
		//КонецЕсли;		
		//Если Объект.Проект = Справочники.Проекты.НайтиПоНаименованию("ПМ", Истина) И Объект.КоличествоКомнат_3 Тогда
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран тип планировки", Объект.Ссылка, "ТипПланировки", "Объект", Отказ);			
		//КонецЕсли;
		////<899444> }
	КонецЕсли;
	//<853881> }	
...
Функция ОпределитьВариантОтчета()
	...
	ИначеЕсли Объект.ТипОбъектаНедвижимости = Перечисления.ВидыОбъектовНедвижимости.КоммерческаяНедвижимость Тогда
		КлючТекущегоВарианта = "Коммерческая";			
	//<890184> }			
	Иначе
		КлючТекущегоВарианта = "ГПТ";
		ВариантГПТ = Истина;
