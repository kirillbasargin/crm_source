...
Процедура СоздатьНаОсновании_ВыдачаДисконтныхКарт(Команда)
	...
	Если Участники.Количество() > 1 Тогда
		...
		КонецЦикла;		
		ОткрытьФорму("Документ.ВыдачаДисконтныйКарт.Форма.ФормаВыбораВыгодоприобретателя", Новый Структура("Запрос, Клиенты", ТекущиеДанные.Ссылка, Клиенты), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОткрытьФорму("Документ.ВыдачаДисконтныйКарт.ФормаОбъекта", Новый Структура("Основание", ТекущиеДанные.Ссылка), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);		
	КонецЕсли;
