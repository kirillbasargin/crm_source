...
Процедура ПослеЗаписи(ПараметрыЗаписи)	
	...
	Если ПараметрыЗаписи.Свойство("ВстречаДляОткрытия") И НЕ ПараметрыЗаписи.ВстречаДляОткрытия = Неопределено Тогда
		ОткрытьФорму("Документ.Встреча.ФормаОбъекта", Новый Структура("Ключ", ПараметрыЗаписи.ВстречаДляОткрытия));
...
&НаКлиенте
Процедура ВзаимодействиеОткрытие(Элемент, СтандартнаяОбработка)
	
	//<828132>, Басаргин (10.09.2018) {
	Если ТипЗнч(Объект.Взаимодействие) = Тип("ДокументСсылка.ЗаявкаНаЗвонок") И ТипЗнч(Объект.Взаимодействие) = Тип("ДокументСсылка.ЗаявкаНаЗвонок") Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Документ.Встреча.ФормаОбъекта", Новый Структура("Ключ, ОткрытиеВзаимодействияИзЗаявкиНаЗвонок", Объект.Взаимодействие, Истина));
	КонецЕсли;	
	//<828132> }
