...
Функция РассчитатьРегресс(Знач ДокументЗапрос) Экспорт
	...
	//<828132>, Басаргин (25.06.2018) { //коммент
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	//"ВЫБРАТЬ
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РАЗНОСТЬДАТ(Взаимодействия.ДатаВзаимодействия, &ТекущаяДата, ДЕНЬ) КАК Регресс
	|ИЗ
	|	РегистрСведений.Взаимодействия КАК Взаимодействия
	|ГДЕ
	|	Взаимодействия.Предмет = &Предмет
	|	И Взаимодействия.Статус = &Статус
	|	И Взаимодействия.АктивностьВзаимодействия = &АктивностьВзаимодействия
	|	И Взаимодействия.ГруппаОтветственного.Наименование В (""Филиал"", ""Центральный офис"", ""Земля розница"", ""Коммерческая недвижимость"")
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("АктивностьВзаимодействия", Перечисления.ТипАктивностиВзаимодействия.Первичное);
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата());
