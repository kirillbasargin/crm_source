...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	РегистрыСведений.Взаимодействия.ЗаписатьЗаявкуНаСделку(ЭтотОбъект);
	//<828132> }
	
	//<852121>, Басаргин (23.10.2018) {
	РежимЗаписиВыдачиДК = РежимЗаписиДокумента.Проведение;
	ДисконтныеКарты_ОновитьДанныеПоСделке(РежимЗаписиВыдачиДК);
	//<852121> }
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//<852121>, Басаргин (24.10.2018) {
	Если ЗначениеЗаполнено(МаркетинговаяКомпания) Тогда	
		ВыгрузкаКомплекснойМК = ПрочитатьСоставКомплекснойКампании(МаркетинговаяКомпания);
		Если ВыгрузкаКомплекснойМК.Количество() Тогда
			ВыдачаДисконтныхКарт = ВыгрузкаКомплекснойМК.НайтиСтроки(Новый Структура("ВыдачаДисконтныхКарт", Истина)).Количество() > 0;			
		Иначе
			ВыдачаДисконтныхКарт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(МаркетинговаяКомпания, "ВыдачаДисконтныхКарт");	
		КонецЕсли;
		
		Если ВыдачаДисконтныхКарт И НЕ ЗначениеЗаполнено(ДисконтнаяКарта) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("По акции " + Строка(МаркетинговаяКомпания) + " должна быть указана «Дисконтная карта». Обратитесь в поддержку CRM.", , , , Отказ);
		КонецЕсли;
	КонецЕсли;
	//<852121> }	
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	//<852121>, Басаргин (23.10.2018) {
	РежимЗаписиВыдачиДК = РежимЗаписиДокумента.ОтменаПроведения;
	ДисконтныеКарты_ОновитьДанныеПоСделке(РежимЗаписиДокумента.ОтменаПроведения);
	//<852121> }
...
Процедура ПроверкаСтатусаОН(Отказ)
	...
	Запрос.Текст = 
		...
		|	Квартирограмма.ОбъектНедвижимости.Представление КАК ОбъектНедвижимостиПредставление,
		|	Квартирограмма.ОбъектСтроительства.Представление КАК ОбъектСтроительстваПредставление
		|ИЗ
		|	РегистрСведений.Квартирограмма КАК Квартирограмма
		|ГДЕ
		|	Квартирограмма.ОбъектНедвижимости В(&ОбъектыНедвижимости)
		|	И (НЕ Квартирограмма.СтатусПула = ЗНАЧЕНИЕ(Перечисление.СтатусыПулов.Открытие)
		|			ИЛИ Квартирограмма.Стоимость = 0
		|			ИЛИ Квартирограмма.ДатаСтартаПродаж = ДАТАВРЕМЯ(1, 1, 1))";
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Объект недвижимости " + Строка(Выборка.ОбъектНедвижимостиПредставление) + " из " + Строка(Выборка.ОбъектСтроительстваПредставление) + " недоступен для продажи!", , , , Отказ);
			//Сообщить("Объект недвижимости " + Строка(Выборка.ОбъектНедвижимости) + " из " + Строка(Выборка.ОбъектНедвижимости.Владелец) + " недоступен для продажи!");
			//Отказ = Истина;
		КонецЦикла;
	КонецЕсли;
...
//<852121>, Басаргин (23.10.2018) {
Процедура ДисконтныеКарты_ОновитьДанныеПоСделке(РежимЗаписи);
	
	Если ЗначениеЗаполнено(МаркетинговаяКомпания) И ЗначениеЗаполнено(ДисконтнаяКарта) Тогда	
		ДисконтнаяКартаОбъект = ДисконтнаяКарта.ПолучитьОбъект();
		Если НЕ ДисконтнаяКартаОбъект = Неопределено Тогда
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				ДисконтнаяКартаОбъект.ЗаявкаНаСделку = Ссылка;
				ДисконтнаяКартаОбъект.Использована = Истина;
			ИначеЕсли РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
				ДисконтнаяКартаОбъект.ЗаявкаНаСделку = Неопределено;
				ДисконтнаяКартаОбъект.Использована = Ложь;				
			КонецЕсли;
			ДисконтнаяКартаОбъект.Записать(РежимЗаписи);	
		КонецЕсли;		
	КонецЕсли;		
