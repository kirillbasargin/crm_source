...
Процедура ДисконтнаяКартаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Документ.ВыдачаДисконтныйКарт.Форма.ФормаВыбораДисконтнойКарты", ПараметрыОткрытияФормы, ЭтаФорма, , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СогласоватьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда	
		...
		Если Записать(Новый Структура("РежимЗаписи", ПредопределенноеЗначение("РежимЗаписиДокумента.Проведение"))) Тогда		
			СогласоватьНаСервереЗавершение(Объект.Ссылка, Объект.Проект, Объект.ВидУсловияОплаты);
			
			//СтатусСделки 							= ПредопределенноеЗначение("Перечисление.СтатусыСделки.Согласование");
			
			//<729503>, Басаргин (02.11.2017) {
			СтатусСделки = ?(ЗначениеРеквизитаОбъекта_ВызовСервера(Объект.ВидУсловияОплаты, "ТребоватьОдобренияИБ"), ПредопределенноеЗначение("Перечисление.СтатусыСделки.СогласованиеИБ"), ПредопределенноеЗначение("Перечисление.СтатусыСделки.СогласованиеЮриста"));
			//<729503> }
...
&НаСервереБезКонтекста
Процедура СогласоватьНаСервереЗавершение(ЗаявкаНаСделку, Проект, ВидУсловияОплаты) Экспорт
	...
	//<729503>, Басаргин (02.11.2017) {
	CRMРаботаСЗаданиямиСервер.СформироватьЗаписиПоСтатусуСделки(ЗаявкаНаСделку, ТекущаяДата(), ?(Ипотека, "СогласованиеИБ", "СогласованиеЮриста"));
	//<729503> }
...
Процедура ОбновитьДоступностьКоманд()
	...
	Если Элементы.Найти("ПостроитьГрафик") <> Неопределено Тогда
		Элементы.ПостроитьГрафик.Доступность 					= ДоступностьЭлементов;
	КонецЕсли;
...
Процедура УправлениеВидимостьюДисконтнойКартыНаСервере(ТолькоВидимость = Ложь)
	...
	Если ДисконтнаяКартаВидимость И НЕ ТолькоВидимость Тогда		
		...
		|ИЗ
		|	Документ.ВыдачаДисконтныйКарт КАК ВыдачаДисконтныйКарт
		|ГДЕ
