...
Процедура СоздатьЗапланированныеДокументыВзаимодействий(ПараметрыЗаписи)
	...
	ПараметрыОткрываемойФормы.Вставить("СтадияЗапроса", ПредопределенноеЗначение("Перечисление.СтадииЗапроса.ПустаяСсылка"));
	//<828132> }	
...
&НаКлиенте
Процедура АнкетаСоздатьНаОсновании(Команда)
	
	Если Модифицированность Тогда	
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОЗаписи", ЭтаФорма, 
			Новый Структура("CallBack", Новый ОписаниеОповещения("СоздатьНаОсновании_Асинхр", ЭтаФорма, Новый Структура("ТипДокумента", "Анкета"))));
		ПоказатьВопрос(Оповещение, "Для продолжения необходимо записать докуменит. Продолжить?", РежимДиалогаВопрос.ДаНет, 0); 
	Иначе
		СоздатьНаОсновании_Асинхр(Неопределено, Новый Структура("ТипДокумента", "Анкета"));
	КонецЕсли;		
	
КонецПроцедуры

&НаКлиенте
Процедура ИпотечнаяЗаявкаСоздатьНаОсновании(Команда)
	
	Если Модифицированность Тогда	
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаОЗаписи", ЭтаФорма, 
			Новый Структура("CallBack", Новый ОписаниеОповещения("СоздатьНаОсновании_Асинхр", ЭтаФорма, Новый Структура("ТипДокумента", "ИпотечнаяЗаявка"))));
		ПоказатьВопрос(Оповещение, "Для продолжения необходимо записать докуменит. Продолжить?", РежимДиалогаВопрос.ДаНет, 0); 
	Иначе
		СоздатьНаОсновании_Асинхр(Неопределено, Новый Структура("ТипДокумента", "ИпотечнаяЗаявка"));
	КонецЕсли;
...
&НаКлиенте
Процедура СоздатьНаОсновании_Асинхр(Результат, ДополнительныеПараметры) Экспорт	
	
	Если НЕ ДополнительныеПараметры.Свойство("ТипДокумента") Тогда
		Возврат;
	КонецЕсли;
		
	Если ДополнительныеПараметры.ТипДокумента = "ИпотечнаяЗаявка" Тогда
		СтруктураПараметров = ПодготовитьПараметрыПередачиОснования(Объект.Ссылка);		
		Если НЕ СтруктураПараметров.ПрикрепленныеФайлы.Количество() Тогда
			ПоказатьПредупреждение(, "Нельзя подать заявку без прикрепленного Согласия на обработку персональных данных. Перейдите на вкладку ""Присоединенные файлы"" и прикрепите СОПД к данному Расчету ипотеки.");
			Возврат;
		КонецЕсли;		
		СтруктураПараметров.СписокБанков.ПоказатьОтметкуЭлементов(Новый ОписаниеОповещения("ПослеОтметкиВыбораБанков", ЭтотОбъект, Новый Структура("ПрикрепленныеФайлы", СтруктураПараметров.ПрикрепленныеФайлы)));
	ИначеЕсли ДополнительныеПараметры.ТипДокумента = "Анкета" Тогда
...
&НаКлиенте
Процедура ПослеОтметкиВыбораБанков(Список, ДополнительныеПараметры) Экспорт
	
	Если НЕ Список = Неопределено Тогда
		Для каждого Элемент из Список Цикл
			Если Элемент.Пометка Тогда
				ПараметрыФормы = Новый Структура("Основание, РекомендуемыйБанк, ТипОбъекта, ПрикрепленныеФайлы", Объект.Ссылка, Элемент.Значение, Объект.ТипОбъекта, ?(ДополнительныеПараметры.Свойство("ПрикрепленныеФайлы"), ДополнительныеПараметры.ПрикрепленныеФайлы, Новый Массив));
				ОткрытьФорму("Документ.ИпотечнаяЗаявка.ФормаОбъекта", ПараметрыФормы, ЭтаФорма, Новый УникальныйИдентификатор);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
...
&НаСервереБезКонтекста
Функция ПодготовитьПараметрыПередачиОснования(РасчетИпотеки)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	|	РасчетИпотекиСписокБанков.Ссылка КАК Ссылка,
	|	ИпотечнаяЗаявка.РекомендуемыйБанк КАК РекомендуемыйБанк
	|ПОМЕСТИТЬ ВТ_ИпотечныеЗаявки
	|ИЗ
	|	Документ.РасчетИпотеки.СписокБанков КАК РасчетИпотекиСписокБанков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИпотечнаяЗаявка КАК ИпотечнаяЗаявка
	|		ПО (ИпотечнаяЗаявка.РасчетИпотеки = РасчетИпотекиСписокБанков.Ссылка)
	|ГДЕ
	|	РасчетИпотекиСписокБанков.Ссылка = &РасчетИпотеки
	|	И РасчетИпотекиСписокБанков.Выбор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_ИпотечныеЗаявки.Ссылка КАК Ссылка,
	|	РасчетИпотекиСписокБанков.Банк КАК Банк,
	|	РасчетИпотекиСписокБанков.Банк.Представление КАК БанкПредставление,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА ВТ_ИпотечныеЗаявки.РекомендуемыйБанк <> РасчетИпотекиСписокБанков.Банк
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК Пометка
	|ИЗ
	|	Документ.РасчетИпотеки.СписокБанков КАК РасчетИпотекиСписокБанков
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ИпотечныеЗаявки КАК ВТ_ИпотечныеЗаявки
	|		ПО (ВТ_ИпотечныеЗаявки.Ссылка = РасчетИпотекиСписокБанков.Ссылка)
	|ГДЕ
	|	РасчетИпотекиСписокБанков.Ссылка = &РасчетИпотеки
	|	И РасчетИпотекиСписокБанков.Выбор
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ИпотечныеЗаявки.Ссылка,
	|	РасчетИпотекиСписокБанков.Банк,
	|	РасчетИпотекиСписокБанков.Банк.Представление";
	
	Запрос.УстановитьПараметр("РасчетИпотеки", РасчетИпотеки);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СписокБанков.Добавить(ВыборкаДетальныеЗаписи.Банк, ВыборкаДетальныеЗаписи.БанкПредставление, ВыборкаДетальныеЗаписи.Пометка);
	КонецЦикла;
	
	Возврат Новый Структура("ПрикрепленныеФайлы, СписокБанков", ПолучитьПрикрепленныеФайлы(РасчетИпотеки), СписокБанков);		
	
КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьПрикрепленныеФайлы(РасчетИпотеки)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РасчетИпотекиПрисоединенныеФайлы.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.РасчетИпотекиПрисоединенныеФайлы КАК РасчетИпотекиПрисоединенныеФайлы
	|ГДЕ
	|	НЕ РасчетИпотекиПрисоединенныеФайлы.ПометкаУдаления
	|	И РасчетИпотекиПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла";
	
	Запрос.УстановитьПараметр("ВладелецФайла", РасчетИпотеки);
