...
Процедура ПриОткрытии(Отказ)
	...
	//<870084>, Басаргин (18.09.2018) {
	Если Объект.Ссылка.Пустая()	Тогда
		ВыполнитьПроверкиНомеровАдресатов();			
	КонецЕсли;
	//<870084> }
...
Процедура АдресатыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если НЕ ТекущиеДанные = Неопределено Тогда	
		СписокНомеровДляКонтакта = ПолучитьСписокНомеровДляКонтакта(ТекущиеДанные.Контакт);
		Если СписокНомеровДляКонтакта.Количество() Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеВыбораЭлемента", ЭтаФорма, Новый Структура("Контакт", ТекущиеДанные.Контакт));
			СписокНомеровДляКонтакта.ПоказатьВыборЭлемента(Оповещение, "Выберите номер клиента " + Строка(ТекущиеДанные.Контакт));			
		КонецЕсли;		
	КонецЕсли;
	//<870084> }
...
Процедура ОткрытьОбработкуРаботаСКлиентами(Команда)
	...
	Если Объект.Адресаты.Количество() > 0 
			И ЗначениеЗаполнено(Объект.Адресаты[0].Контакт) Тогда
		
		ПараметрыФормы = Новый Структура("ПозиционированиеНаКлиенте", Объект.Адресаты[0].Контакт);
...
//++ Юкаев Роман 20180206 ( //773222
&НаКлиенте
Процедура МаркетинговоеМероприятиеПриИзменении(Элемент)
	
	Объект.ТекстСообщения = ПолучитьТекстСообщения(Объект.МаркетинговоеМероприятие);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьТекстСообщения(МаркетинговоеМероприятие)
	Возврат МаркетинговоеМероприятие.ТекстРассылки;
КонецФункции
//-- Юкаев Роман 20180206 )

//<870084>, Басаргин (18.09.2018) {
&НаКлиенте
Процедура ВыполнитьПроверкиНомеровАдресатов()

	Если НЕ Объект.Адресаты.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого СтрокаАдресат Из Объект.Адресаты Цикл
		СписокНомеровДляКонтакта = ПолучитьСписокНомеровДляКонтакта(СтрокаАдресат.Контакт);
		Если СписокНомеровДляКонтакта.Количество() Тогда
			Оповещение = Новый ОписаниеОповещения("ПослеВыбораЭлемента", ЭтаФорма, Новый Структура("Контакт", СтрокаАдресат.Контакт));
			СписокНомеровДляКонтакта.ПоказатьВыборЭлемента(Оповещение, "Выберите номер клиента " + Строка(СтрокаАдресат.Контакт));			
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ПолучитьСписокНомеровДляКонтакта(Контакт) 
	
	СписокВыбораНомеров = Новый СписокЗначений();
	...
	|ГДЕ
	|	КлиентыКонтактнаяИнформация.Ссылка = &Контакт
	|	И КлиентыКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.Телефон)
	|	И КлиентыКонтактнаяИнформация.Вид В (ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ОсновнойТелефонКлиента), ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ТелефонКлиента))";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СписокВыбораНомеров.Добавить(ВыборкаДетальныеЗаписи.Представление, ФорматироватьНомер(ВыборкаДетальныеЗаписи.Представление));
	КонецЦикла;
	
	Возврат СписокВыбораНомеров;
...
Процедура ПослеВыбораЭлемента(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ВыбранныйЭлемент = Неопределено Тогда
        НомерДляОтправки = ВыбранныйЭлемент.Значение;
		КакСвязаться = ВыбранныйЭлемент.Представление;
		НайденныеСтроки = Объект.Адресаты.НайтиСтроки(Новый Структура("Контакт", ДополнительныеПараметры.Контакт));
		Для каждого Найденная Из НайденныеСтроки Цикл
			Найденная.НомерДляОтправки = НомерДляОтправки;
			Найденная.КакСвязаться = КакСвязаться;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура АдресатыКонтактОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)

	СписокНомеровДляКонтакта = ПолучитьСписокНомеровДляКонтакта(ВыбранноеЗначение);
	Если СписокНомеровДляКонтакта.Количество() Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеВыбораЭлемента", ЭтаФорма, Новый Структура("Контакт", ВыбранноеЗначение));
		СписокНомеровДляКонтакта.ПоказатьВыборЭлемента(Оповещение, "Выберите номер клиента " + Строка(ВыбранноеЗначение));			
	КонецЕсли;	
