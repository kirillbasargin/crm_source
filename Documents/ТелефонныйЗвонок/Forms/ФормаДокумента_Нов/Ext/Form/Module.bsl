...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Объект.Ссылка.Пустая() И (ЭтоМенеджерГПТ ИЛИ ЭтоМенеджерФилиала) Тогда
		...
		ИначеЕсли НЕ АктуальныеПараметрыЗапроса.Взаимодействие = Объект.Ссылка И (НЕ ЗначениеЗаполнено(Объект.СтатусЗапроса) ИЛИ НЕ ЗначениеЗаполнено(Объект.РасшифровкаСтатусаЗапроса)) Тогда
			ДоступностьСтатусаРасшифровкиСтатуса = Истина;
		КонецЕсли;	
