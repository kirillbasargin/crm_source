...
Процедура ВыполнитьЗаписьВРегистр(Взаимодействие, СтруктураЗаписи, ПересчетПриПометкеУдаления = Ложь) Экспорт
	...
	Попытка	
		...
		Если НЕ ТипЗнч(Взаимодействие) = Тип("ДокументСсылка.ЗаявкаНаСделку") И НЕ ТипЗнч(Взаимодействие) = Тип("СправочникСсылка.Сделки") Тогда		
			...
			КонецЕсли;
			//<891309>, Басаргин (30.10.2018) {
			Если Запись.СтадияЗапроса = Перечисления.СтадииЗапроса.Сделка Тогда
				Запись.СтатусЗапроса = Неопределено;
				Запись.РасшифровкаСтатусаЗапроса = Неопределено;
				Запись.ДатаПоследнегоИзменения = Неопределено;
				ВзаимодействиеОбъект = Взаимодействие.ПолучитьОбъект();
				Если НЕ ВзаимодействиеОбъект = Неопределено Тогда
					ВзаимодействиеОбъект.СтатусЗапроса = Неопределено;
					ВзаимодействиеОбъект.РасшифровкаСтатусаЗапроса = Неопределено;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("Взаимодействия.ВыполнитьЗаписьВРегистр",
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.РегистрыСведений.Взаимодействия,
			Взаимодействие,
			"Ошибка в транзакции записи взаимодействия." + Символы.ПС + " Описание ошибки: " + ОписаниеОшибки());			
...
Процедура ЗаписатьЗаявкуНаСделку(ЗаявкаНаСделку, ВыполнитьПолныйПересчет = Истина) Экспорт
	...
	СтруктураЗаписи.ДатаВзаимодействия = КонецДня(ЗаявкаНаСделку.Дата);	
	СтруктураЗаписи.ДатаПоследнегоИзменения = КонецДня(ЗаявкаНаСделку.Дата); 	
...
Процедура ЗаполнитьСтадиюСтатусРасшифровкуСтатуса(Взаимодействие) Экспорт
	...
	Если Взаимодействие.ТипВзаимодействия = Перечисления.ТипыВзаимодействий.Расторжение Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаявкаНаСделку КАК ЗаявкаНаСделкуДокумент
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.Запрос КАК Запрос
		|			ПО ЗаявкаНаСделкуДокумент.ДокументОснование = Запрос.Ссылка
		|		ПО Сделки.ЗаявкаНаСделку = ЗаявкаНаСделкуДокумент.Ссылка
		|ГДЕ
		|	НЕ Сделки.СтатусСделки = ЗНАЧЕНИЕ(Перечисление.СтатусыСделокСправочник.Расторгнута)
		|	И Запрос.Ссылка = &Ссылка";
		
		Запрос.УстановитьПараметр("Ссылка", Взаимодействие.Предмет); 
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Если ВыборкаДетальныеЗаписи.Следующий() Тогда
				...
				Взаимодействие.СтатусЗапроса = Неопределено; 
...
Процедура ПересчитатьЗаписиПоЗапросу(Предмет, ПересчитыватьДляСделок = Ложь) Экспорт
	...
	Исключение	
		...
		ЗаписьЖурналаРегистрации("Взаимодействия.ПересчитатьЗаписиПоЗапросу",
			УровеньЖурналаРегистрации.Ошибка,
			Метаданные.РегистрыСведений.Взаимодействия,
			Предмет,
			"Ошибка в транзакции пересчета взаимодействий по Предмету." + Символы.ПС + " Описание ошибки: " + ОписаниеОшибки());			
