...
Процедура ОтправитьПоПочте(ПутьФайлаОтчета)
	...
	Для каждого КлиентЗапроса Из КлиентыЗапроса Цикл
		...
		Если НЕ МассивПолучателей.Количество() Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указан email клиента");
			Возврат;
		КонецЕсли;
		...
		|<p>В продолжение нашего разговора направляю подобранные варианты интересующих Вас квартир. <br>
...
Процедура КлиентыЗапросаEmailПриИзмененииНаСервере(Клиент, Email)
	...
	Если EmailКлиента = Email ИЛИ Email = "" Тогда
		Возврат;		
	КонецЕсли;
	...
	Если НЕ КлиентОбъект = Неопределено Тогда		
		...
		Если НайденныеСтроки.Количество() Тогда
			Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				НайденнаяСтрока.АдресЭП = ?(НЕ ПустаяСтрока(EmailКлиента), EmailКлиента + ";" + Email, Email);
				НайденнаяСтрока.Представление = НайденнаяСтрока.АдресЭП;			
...
&НаКлиенте
Процедура КлиентыЗапросаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если Элемент.ТекущийЭлемент.Имя = "КлиентыЗапросаEmail" Тогда
		ТекущиеДанные = Элементы.КлиентыЗапроса.ТекущиеДанные;
		Если НЕ ТекущиеДанные = Неопределено Тогда
			Email = ТекущиеДанные.Email;
			ТекущиеДанные.Email = "";
		КонецЕсли;		
	КонецЕсли;	
...
Процедура ОбновитьНаСервере()
	...
	Для каждого Элемент Из КлиентыЗапроса Цикл
		Элемент.Email = ПолучитьEmailКлиента(Элемент.Клиент);			
	КонецЦикла;
