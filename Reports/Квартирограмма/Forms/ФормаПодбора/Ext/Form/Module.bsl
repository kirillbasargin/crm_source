...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("КлючТекущегоВарианта") И ЗначениеЗаполнено(Параметры.КлючТекущегоВарианта) Тогда
		КлючТекущегоВарианта = Параметры.КлючТекущегоВарианта;
	Иначе
		КлючТекущегоВарианта = ПолучитьВариантПоУмолчанию();
		//ВариантГПТ = Истина;
		//Параметры_ВариантГПТ = ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("ВариантГПТ");
		//Если НЕ Параметры_ВариантГПТ = Неопределено Тогда
		//	Параметры_ВариантГПТ.Значение = ВариантГПТ;
		//КонецЕсли;
		//Параметры_ВариантГПТ = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ВариантГПТ");	
		//Если НЕ Параметры_ВариантГПТ = Неопределено Тогда
		//	Параметры_ВариантГПТ.Значение = ВариантГПТ;
		//	Параметры_ВариантГПТ.Использование = Истина;
		//КонецЕсли;	
	КонецЕсли;	
	...
	//<ИзЗапроса>, Басаргин (25.10.2017) {
	//Параметры_ВариантГПТ = ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("ВариантГПТ");
	//Если Параметры.Свойство("ВариантГПТ") И НЕ Параметры_ВариантГПТ = Неопределено Тогда
	//	Параметры_ВариантГПТ.Значение = Параметры.ВариантГПТ;
	//КонецЕсли;
	
	//++ Юкаев Роман 20180517 (//Не используется в отчете
	//Параметры_ЗУ_СГазом = ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("ВариантГПТ");
	//Если НЕ Параметры_ЗУ_СГазом = Неопределено Тогда
	//	Параметры_ЗУ_СГазом.Значение = CRMСервер.ПолучитьНазначение_СГазом();
	//КонецЕсли;
	//
	//Параметры_ЗУ_БезГаза = ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("ВариантГПТ");
	//Если НЕ Параметры_ЗУ_БезГаза = Неопределено Тогда
	//	Параметры_ЗУ_БезГаза.Значение = CRMСервер.ПолучитьНазначение_БезГаза();
	//КонецЕсли;
	//-- Юкаев Роман 20180517 )
		
	ТекущаяДата = ТекущаяДата();
	
	Параметры_ТекущаяДата = ОтчетОбъект.СхемаКомпоновкиДанных.Параметры.Найти("ТекущаяДата");
	Если НЕ Параметры_ТекущаяДата = Неопределено Тогда
		Параметры_ТекущаяДата.Значение = ТекущаяДата;
	КонецЕсли;	
	
	//Параметры_ВариантГПТ = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ВариантГПТ");	
	//Если Параметры.Свойство("ВариантГПТ") И НЕ Параметры_ВариантГПТ = Неопределено Тогда
	//	ВариантГПТ = Параметры.ВариантГПТ;
	//	Параметры_ВариантГПТ.Значение = ВариантГПТ;
	//	Параметры_ВариантГПТ.Использование = Истина;
	//КонецЕсли;	
	
	Параметры_ТекущаяДата = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ТекущаяДата");	
	Если НЕ Параметры_ТекущаяДата = Неопределено Тогда
		Параметры_ТекущаяДата.Значение = ТекущаяДата;
		Параметры_ТекущаяДата.Использование = Истина;
	КонецЕсли;
	
	Параметры_ЗУ_СГазом = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ЗУ_СГазом");	
	Если НЕ Параметры_ЗУ_СГазом = Неопределено Тогда
		Параметры_ЗУ_СГазом.Значение = CRMСервер.ПолучитьНазначение_СГазом();
		Параметры_ЗУ_СГазом.Использование = Истина;
	КонецЕсли;
	
	Параметры_ЗУ_БезГаза = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ЗУ_БезГаза");	
	Если НЕ Параметры_ЗУ_БезГаза = Неопределено Тогда
		Параметры_ЗУ_БезГаза.Значение = CRMСервер.ПолучитьНазначение_БезГаза();
		Параметры_ЗУ_БезГаза.Использование = Истина;
	КонецЕсли;	
	//<ИзЗапроса> }		
	...
	//EXTCODE Шумилин Сергей 08.08.2017 {{ --->
	...
	//EXTCODE Шумилин Сергей 16.03.2017 {{ --->
	ФормаПодбора = Параметры.СвернутьПанельНастроек;
	...
	Если Параметры.Свойство("КлиентыЗапроса") И Параметры.КлиентыЗапроса.Количество() Тогда
		КлиентыЗапроса.ЗагрузитьЗначения(Параметры.КлиентыЗапроса);
	КонецЕсли;
	//<872403> }
...
Процедура ПриОткрытии(Отказ)
	...
	//EXTCODE Шумилин Сергей 16.03.2017 {{ --->
	Если НЕ ЭтаФорма.ФормаПараметры.СформироватьПриОткрытии = Неопределено И ЭтаФорма.ФормаПараметры.СформироватьПриОткрытии Тогда
		Элементы.СтраницыНастройки.Видимость	= Ложь;
		Элементы.ГруппаРазделитель.Видимость 	= Истина;
		Элементы.ПанельНастроек.Ширина 			= 0;
	КонецЕсли;	
	//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
	...
	Если НастройкиОтчета.Контекстный ИЛИ (НЕ ЭтаФорма.ФормаПараметры.СформироватьПриОткрытии = Неопределено И ФормаПараметры.СформироватьПриОткрытии=Истина) Тогда
	...
	Элементы.ПечатьТаблицы.Видимость = ВидимостьКнопкиПечатиТаблицы(Вариант);
	//-- Юкаев Роман 20180404 )
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("УправляемаяФорма") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если СуффиксФормыИсточника = ВРег("ФормаНастроекОтчета")
				...
				Или СуффиксФормыИсточника = ВРег("ФормаВариантаОтчетаCRM")
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	НастройкиОтчета.Вставить("ЭтоОтчетCRM", 
	НЕ ОтчетОбъект.Метаданные().Реквизиты.Найти("ЭтоОтчетCRM")=Неопределено И ОтчетОбъект.ЭтоОтчетCRM);
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Данные.Поле = "МаксимальнаяСкидка"
		ИЛИ Данные.Поле = "Стоимость" Тогда
		СтандартнаяОбработка = Ложь;
		ВыполнитьРасшифровкуПоСкидке(Расшифровка, Данные, Данные.Значение);
	Иначе
...
//++ Юкаев Роман 20171214 (
&НаКлиенте
Процедура ВыполнитьРасшифровкуПоСкидке(Расшифровка, Данные, ОН)
	
	Результат = СформироватьРасшифровкуНаСервере(Расшифровка, Данные, ОН);
	Результат.ОтображатьСетку = Ложь;
	Результат.ОтображатьЗаголовки = Ложь;
	Результат.Показать("Расшифровка максимальных скидок");

КонецПроцедуры

&НаСервере
Функция СформироватьРасшифровкуНаСервере(Расшифровка, Данные, ОН)
	
	ЭтотОтчет = РеквизитФормыВЗначение("Отчет");
	СхемаКомпоновки = ЭтотОтчет.ПолучитьМакет("РасшифровкаМаксимальнойСкидки");
	
	Настройки = СхемаКомпоновки.НастройкиПоУмолчанию;
		
	ДанныеРасшифровки = ПолучитьИзВременногоХранилища(ОтчетДанныеРасшифровки);
	
	Для Каждого Эл Из СхемаКомпоновки.Параметры Цикл
		Эл.Значение = Неопределено;
	КонецЦикла;
	
	Для Каждого ЭлементНастройкиОтбора Из Данные.Настройки.Отбор.Элементы Цикл
		
		Если ЭлементНастройкиОтбора.Использование И НЕ ТипЗнч(ЭлементНастройкиОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			ПолеКомпоновки = СхемаКомпоновки.Параметры.Найти(ЭлементНастройкиОтбора.ЛевоеЗначение);
			Если ПолеКомпоновки <> Неопределено Тогда
				Если ПолеКомпоновки.Заголовок = "КоличествоСекций" Тогда
					Если ЭлементНастройкиОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно Тогда
						ПолеКомпоновки.Значение = 1;
					Иначе
						ПолеКомпоновки.Значение = 0;
					КонецЕсли;
				ИначеЕсли ПолеКомпоновки.Заголовок = "Площадь" Тогда
					ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ПлощадьБольшеИлиРавно");
					ПолеВарианта.Значение = ЭлементНастройкиОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
				ИначеЕсли ПолеКомпоновки.Заголовок = "Отделка" Тогда
					Если ПолеКомпоновки.Значение = Неопределено Тогда
						ПолеКомпоновки.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					Иначе
						ПолеКомпоновки.Значение = Неопределено;
					КонецЕсли;
				ИначеЕсли ПолеКомпоновки.Заголовок = "ВидМатериала" Тогда
					Если ПолеКомпоновки.Значение = Неопределено Тогда
						ПолеКомпоновки.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					Иначе
						ПолеКомпоновки.Значение = Неопределено;
					КонецЕсли;
				ИначеЕсли ПолеКомпоновки.Заголовок = "СтоимостьСУчетомСкидки" Тогда
					Если ЭлементНастройкиОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно Тогда
						ПолеВарианта = СхемаКомпоновки.Параметры.Найти("СтоимостьСНизу");
						ПолеВарианта.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					Иначе
						ПолеВарианта = СхемаКомпоновки.Параметры.Найти("СтоимостьСВерху");
						ПолеВарианта.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					КонецЕсли;
				ИначеЕсли ПолеКомпоновки.Заголовок = "Этаж" Тогда
					Если ЭлементНастройкиОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно Тогда
						ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ЭтажСНизу");
						ПолеВарианта.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					ИначеЕсли ЭлементНастройкиОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно Тогда
						ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ЭтажСВерху");
						ПолеВарианта.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;
					Иначе
						Если ЭлементНастройкиОтбора.Представление = "НЕ первый этаж" Тогда
							ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ПервыйЭтаж");
							ПолеВарианта.Значение = Истина;
						Иначе
							ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ПоследнийЭтаж");
							ПолеВарианта.Значение = Истина;
						КонецЕсли;
					КонецЕсли;
				ИначеЕсли ПолеКомпоновки.Заголовок = "Маркетинговая компания" Тогда
					Если Данные.Поле = "Стоимость" Тогда
						ПолеКомпоновки.Значение = Неопределено;
					Иначе
						Если ТипЗнч(ЭлементНастройкиОтбора.ПравоеЗначение) = Тип("СписокЗначений") Тогда
							ПолеКомпоновки.Значение = ?(ЭлементНастройкиОтбора.ПравоеЗначение.Количество(), ЭлементНастройкиОтбора.ПравоеЗначение[0].Значение, Неопределено);
						Иначе	
							ПолеКомпоновки.Значение = ЭлементНастройкиОтбора.ПравоеЗначение;	
						КонецЕсли;						
					КонецЕсли;					
	...
	Для Каждого ЭлементОтбор Из Данные.Отбор Цикл
		Если СхемаКомпоновки.Параметры.Найти(ЭлементОтбор.Ключ) <> Неопределено Тогда
			СхемаКомпоновки.Параметры[ЭлементОтбор.Ключ].Значение = ЭлементОтбор.Значение;
		КонецЕсли;
	КонецЦикла;                 
	
	//Параметры_ВариантГПТ = Данные.Настройки.ПараметрыДанных.Элементы.Найти("ВариантГПТ");	
	//Если НЕ Параметры_ВариантГПТ = Неопределено Тогда
	//	ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ВариантГПТ");
	//	ПолеВарианта.Значение = ВариантГПТ;
	//КонецЕсли;
	
	Родители = ДанныеРасшифровки.Элементы.Получить(Расшифровка).ПолучитьРодителей();
	Если ТипЗнч(Родители[0]) = Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка") Тогда
		ПолеВарианта = СхемаКомпоновки.Параметры.Найти("ОбъектНедвижимости");
		ПолеВарианта.Значение = ОН;
	КонецЕсли;
	
	ДанныеНовойРасшифровки = Новый ДанныеРасшифровкиКомпоновкиДанных;
	Компоновщик = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = Компоновщик.Выполнить(СхемаКомпоновки, Настройки, ДанныеНовойРасшифровки);
	
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, , ДанныеНовойРасшифровки, Истина);
...
Процедура ОтчетТабличныйДокументОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если Данные.ЭтоСсылка Тогда
		...
		ДополнительныеДействия.Добавить("Шахматка", "Шахматка по ОС");
		//EXTCODE Шумилин Сергей 15.03.2017 <--- }} 
		//<869825>, Басаргин (19.09.2018) {
		ДополнительныеДействия.Добавить("ЗаявкаНаВывод", "Заявка на вывод");		
		//<869825> }
...
Процедура ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	Если ВыполненноеДействие = ДействиеОбработкиРасшифровкиКомпоновкиДанных.Отфильтровать Тогда
		...
		ОтметитьНеактуальным();
	//<869825>, Басаргин (19.09.2018) {
	ИначеЕсли ВыполненноеДействие = "ЗаявкаНаВывод" Тогда
		...
		ОткрытьФорму("Документ.ЗаявкаНаВыводОНВПродажу.ФормаОбъекта", Новый Структура("ОбъектНедвижимости", ЗначениеОН));
	//<869825> }	
	//EXTCODE Шумилин Сергей 15.03.2017 {{ ---> 
	ИначеЕсли ВыполненноеДействие = "Шахматка" Тогда		
		//Ещё один серверный вызов, но, чтобы не переписывать больше кода и, учитывая, что это отчёт - оставляем такой вариант
		ЗначениеОН = CRMОтчетыВызовСервера.ПолучитьЗначениеРасшифровкиБезАнализа("ОбъектНедвижимости", Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);	
		Подъезд_Ссылка = CRMОтчетыВызовСервера.ПолучитьЗначениеРасшифровкиБезАнализа("Подъезд_Ссылка", Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);
		Этаж = CRMОтчетыВызовСервера.ПолучитьЗначениеРасшифровкиБезАнализа("_Этаж", Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);
		ВсегоЭтажей = CRMОтчетыВызовСервера.ПолучитьЗначениеРасшифровкиБезАнализа("ВсегоЭтажей", Расшифровка, ЭтаФорма.ОтчетДанныеРасшифровки);
		Парамтры_Отбор = Новый Структура("ОбъектСтроительства",Данные.Значение);
		//Если ЗначениеЗаполнено(Подъезд) Тогда
		//	Парамтры_Отбор.Вставить("Подъезд", Подъезд);
		//КонецЕсли;		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("СформироватьПриОткрытии",		Истина);
		ПараметрыФормы.Вставить("Отбор",						Парамтры_Отбор);
		ПараметрыФормы.Вставить("ОбъектНедвижимостиВыделение",	ЗначениеОН);
		ПараметрыФормы.Вставить("Подъезд",	Подъезд_Ссылка);
		ПараметрыФормы.Вставить("ЭтажПерехода",	Этаж);
		ПараметрыФормы.Вставить("ВсегоЭтажей",	ВсегоЭтажей);		
...
Процедура ДобавитьОформление(Поле, Оформление)
	...
	ОтчетыCRMКлиентСервер.ДобавитьУсловноеОформление(Отчет.КомпоновщикНастроек.Настройки, Поле, Оформление);
	ОтметитьНеактуальным();
...
Процедура ДобавитьСортировку(Поле, Направление)
	...
	//EXTCODE Шумилин Сергей 02.03.2017 {{ --->
	//ОтметитьНеактуальным();
...
Процедура ОтчетТабличныйДокументПриАктивизацииОбласти(Элемент)
	...
	//EXTCODE Шумилин Сергей 03.03.2017 {{ --->
	РассчитыватьСуммуЯчеек = Ложь;
	Если НЕ НастройкиОтчета.Свойство("РассчитыватьСуммуЯчеек", РассчитыватьСуммуЯчеек)
			И НЕ РассчитыватьСуммуЯчеек Тогда
	//EXTCODE Шумилин Сергей 03.03.2017 <--- }} 
...
Функция СписокДополнительныхКолонок()
	...
	Для каждого Показатель Из Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		...
		Если НастройкиОтчета.Свойство("ТолькоРесурсыОстатков") И НастройкиОтчета.ТолькоРесурсыОстатков Тогда
			ПолеСхемы = ОтчетыCRM.ПолеСхемы(Схема, ИмяПоля);
...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	...
	//EXTCODE Шумилин Сергей 16.03.2017 {{ --->
	Если Элементы.СтраницыНастройки.Видимость Тогда
		СвернутьРазвернутьПанельНастроек("");	
	КонецЕсли;
	//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
...
Процедура ЗапомнитьНастройки(Команда)
	...
	Форма = ОткрытьФорму("ОбщаяФорма.СохранениеВариантаОтчетаCRM", ПараметрыОткрытия, ЭтаФорма);
...
//<872403>, Басаргин (26.09.2018) {
&НаКлиенте
Процедура ОтправитьКлиенту(Команда)
	
	Если НЕ РезультатПодбора.Количество() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Нет подобранных объекттов");
		Возврат;
	КонецЕсли;
	
	//ПараметрыФормы = Новый Структура("АдресТаблицыПодбора, КлиентыЗапроса", ПоместитьВоВременноеХранилище(РезультатПодбора.Выгрузить()), КлиентыЗапроса);
	ОткрытьФорму("Отчет.Квартирограмма.Форма.ФормаОтправкиОтчетаКлиенту", ПолучитьПараметрыФормы(), ЭтаФорма, УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПараметрыФормы()
	Возврат Новый Структура("АдресТаблицыПодбора, КлиентыЗапроса, Проект", ПоместитьВоВременноеХранилище(РезультатПодбора.Выгрузить()), КлиентыЗапроса, ФормаПараметры.Отбор.Получить("Проект"));	
КонецФункции
//<872403> }
...
Процедура ВсеНастройки(Команда)
	...
	Имя = "Отчет.Квартирограмма" + ".ФормаНастроек";
...
&НаКлиенте
Процедура НастройкиСКД(Команда)
	РедактироватьСКД();
КонецПроцедуры
...
Функция ТаблицаВариантов(ТолькоВидимые = Истина)
	...
	ИначеЕсли ТолькоВидимые Тогда
		...
		Запрос.Параметры.Вставить("ОтключенныеПредопределенныеВарианты", ОтчетыCRMПовтИсп.ПредопределенныеВариантыНедоступныеПоФО());
		Запрос.Параметры.Вставить("ОтключенныеВарианты", ОтчетыCRMПовтИсп.ВариантыНедоступныеПоФО());
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиВариантовОтчетовCRM КАК НастройкиВариантовОтчетовCRM
		|		ПО НастройкиВариантовОтчетовCRM.Вариант = ВариантыОтчетов.Ссылка
		|ГДЕ
...
Процедура МодифицироватьСхему(СхемаКомпоновкиДанных)
	...
	ОтчетыCRM.СтандартизироватьСхему(СхемаКомпоновкиДанных);
	ОтчетыCRM.ДобавитьВычисляемыеПоля(СхемаКомпоновкиДанных);
	
	// Запрет непосредственного использования поля ДинамическийПериод 
	Поле = ОтчетыCRM.ПолеСхемы(СхемаКомпоновкиДанных, "ДинамическийПериод");
...
Процедура ОпределитьПараметрыОтчетаПоУмолчанию()
	...
	ОтчетыCRM.ИнициализироватьНастройкиОтчета(НастройкиОтчета);
	...
	Для Каждого Итератор Из ТаблицаВариантов(Ложь) Цикл
		...
		ОтчетыCRM.ИнициализироватьНастройкиВарианта(НастройкиВариантов[Итератор.КлючВарианта]);
	КонецЦикла;
	Если НастройкиОтчета.ЭтоОтчетCRM Тогда
	...
	Иначе
		...
		ОтчетыCRM.ИнициализироватьНастройкиВарианта(СтруктураНастроек);
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Попытка
		Если ФоновоеЗаданиеИдентификатор <> Неопределено Тогда
			ФоновоеЗаданиеОтменить(ФоновоеЗаданиеИдентификатор);
		КонецЕсли; 
		РезультатФоновогоЗадания = CRMСервер.ЗапуститьВыполнениеВФоне(
...
Процедура Сформировать()
	...
	//<>, Басаргин (25.10.2017) {
	//Параметры_ВариантГПТ = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ВариантГПТ");	
	//Если НЕ Параметры_ВариантГПТ = Неопределено Тогда
	//	Параметры_ВариантГПТ.Значение = ВариантГПТ;
	//	Параметры_ВариантГПТ.Использование = Истина;
	//КонецЕсли;	
	
	Параметры_ТекущаяДата = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ТекущаяДата");	
	Если НЕ Параметры_ТекущаяДата = Неопределено Тогда
		Параметры_ТекущаяДата.Значение = ТекущаяДата();
		Параметры_ТекущаяДата.Использование = Истина;			
	КонецЕсли;
	
	Параметры_ЗУ_СГазом = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ЗУ_СГазом");	
	Если НЕ Параметры_ЗУ_СГазом = Неопределено Тогда
		Параметры_ЗУ_СГазом.Значение = CRMСервер.ПолучитьНазначение_СГазом();
		Параметры_ЗУ_СГазом.Использование = Истина;
	КонецЕсли;
	
	Параметры_ЗУ_БезГаза = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ЗУ_БезГаза");	
	Если НЕ Параметры_ЗУ_БезГаза = Неопределено Тогда
		Параметры_ЗУ_БезГаза.Значение = CRMСервер.ПолучитьНазначение_БезГаза();
		Параметры_ЗУ_БезГаза.Использование = Истина;
	КонецЕсли;		
...
Процедура ПоказатьОшибкиФормирования(ИнформацияОбОшибке)
	...
	Если Не ПустаяСтрока(ПодробноеПредставлениеОшибки) Тогда
		ВариантыОтчетов.ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ПодробноеПредставлениеОшибки, НастройкиОтчета.ВариантСсылка);
	КонецЕсли;
КонецПроцедуры
		...
		Если ТипЗнч(ЭлементИсточник)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(ЭлементИсточник)=Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			...
			Для каждого ПолеИсточник Из ЭлементИсточник.ПоляГруппировки.Элементы Цикл
				...
				Если ТипЗнч(ПолеИсточник)=Тип("ПолеГруппировкиКомпоновкиДанных") Тогда
					...
					ПолеСхемы = ОтчетыCRM.ПолеСхемы(СхемаКомпоновкиДанных, ПолеИсточник.Поле);
		...
		Если ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") 
			...
			Для каждого Поле Из Элемент.ПоляГруппировки.Элементы Цикл
				...
				Если НЕ ИмяПоля="ДинамическийПериод" И НЕ ИмяПоля="МесяцГода" И НЕ ИмяПоля="ДеньМесяцГода" И НЕ ИмяПоля="НеделяГода" Тогда
					...
					ПолеСхемы = ОтчетыCRM.ПолеСхемы(СхемаКД, Поле.Поле);
...
Процедура ОбновитьНастройкиДиаграммыНаФорме()
	...
	Если ИмяТочки="ДинамическийПериод" Тогда
		...
		Если ПараметрДанныхПериодичность=Неопределено ИЛИ НЕ ПараметрДанныхПериодичность.Использование ИЛИ НЕ ЗначениеЗаполнено(ПараметрДанныхПериодичность.Значение) Тогда
			ИмяТочки = CRMОтчеты.ПолучитьЗначениеПериодичности(Период.ДатаНачала, Период.ДатаОкончания);
		Иначе
	...
	Для каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Стр.Тип=ТипПараметр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			ОтчетыCRM.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, Неопределено);
		ИначеЕсли Стр.Тип=ТипФильтр() Тогда
			...
			Иначе
				ПараметрыВыбора = Неопределено;
			КонецЕсли; 
			ОтчетыCRM.ДобавитьЭлементФильтр(Стр, ЭтаФорма, ПараметрыВыбора, СвязиПараметровВыбора);
		КонецЕсли; 
		Стр.Создан = Истина;
	КонецЦикла;
	 ...
	 Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		 Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			 ...
			 Если ЗначениеЗаполнено(Стр.Поле) Тогда
				 ПолеГруппировки = ОтчетыCRM.ПолеСхемы(Схема, Стр.Поле);
			 ...
			 Иначе
				 Для каждого ПодСтр Из Стр.ПолучитьЭлементы() Цикл
					 Если НЕ ПодСтр.Тип=ТипРеквизит() Тогда
						 Продолжить;
					 КонецЕсли; 
					 ПолеГруппировки = ОтчетыCRM.ПолеСхемы(Схема, ПодСтр.Поле);
					 Если ТипЗнч(ПолеГруппировки)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И НЕ ПолеГруппировки.Роль.НомерПериода=0 И ПолеГруппировки.Роль.НомерПериода<ПолеСхемы.Роль.НомерПериода Тогда
						 Индекс = ?(Индекс=Неопределено, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр), Мин(Индекс, ПоляНастроек.ПолучитьЭлементы().Индекс(Стр)));
						 Прервать;
					 КонецЕсли;
...
Функция ПроверитьСовместимостьГруппировокКолонок(ИмяПоляПроверки = "")
	...
	Если ЗначениеЗаполнено(ПолеГруппировкиКолонок) Тогда
		ГруппировкаКолонокСхемы = ОтчетыCRM.ПолеСхемы(Схема, ПолеГруппировкиКолонок);
	...
	Если ЗначениеЗаполнено(ИмяСравнения) Тогда
		СравнениеСхемы = ОтчетыCRM.ПолеСхемы(Схема, ИмяСравнения);
	...
	Для каждого ИмяПоля Из МассивПолей Цикл
		Поле = ОтчетыCRM.ПолеСхемы(Схема, ИмяПоля);
...
Процедура ДобавитьОписанияГруппировокРекурсивно(ЭлементыСтруктуры, ЭтоСтрока = Истина)
	...
	Для каждого Элемент Из ЭлементыСтруктуры Цикл
		...
		ИначеЕсли ЭтоСтрока И (ТипЗнч(Элемент)=Тип("ГруппировкаКомпоновкиДанных") ИЛИ ТипЗнч(Элемент)=Тип("ГруппировкаТаблицыКомпоновкиДанных")) Тогда
			...
			//EXTCODE Шумилин Сергей 22.02.2017 {{ --->
			ИначеЕсли Элемент.ПоляГруппировки.Элементы.Количество()=0 Тогда
				//Поле = Элемент.ПоляГруппировки.Элементы[0];
				//Стр.Поле = Строка(Поле.Поле);
				МассивПолейДетальныхЗаписей = Новый Массив;
				Стр.ДополнительныеПараметры.Вставить("ТипГруппировки", ТипГруппировкиКомпоновкиДанных.Элементы);				
				Для каждого ПолеДетальныхЗаписей Из Элемент.Выбор.Элементы Цикл
					МассивПолейДетальныхЗаписей.Добавить(ПолеДетальныхЗаписей.Поле);
				КонецЦикла;
				Стр.ДополнительныеПараметры.Вставить("ВыбранныеПоля", МассивПолейДетальныхЗаписей);
			//EXTCODE Шумилин Сергей 22.02.2017 <--- }} 
			Иначе
...
Функция ПроверитьВозможностьДобавленияГруппировки(Стр, СтрГруппа)
	...
	ПолеСхемы = ОтчетыCRM.ПолеСхемы(Схема, Стр.Поле);
	Если ТипЗнч(ПолеСхемы)=Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") И ПолеСхемы.Роль.НомерПериода>0 Тогда
		Для каждого ПодСтр Из СтрГруппа.ПолучитьЭлементы() Цикл
			ВложенноеПоле = ОтчетыCRM.ПолеСхемы(Схема, ПодСтр.Поле);
...
Процедура ДобавитьПереключательИерархииГруппировки(Стр, ИмяРеквизита, Группа)
	...
	Если ДоступноеПоле=Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если НЕ ОтчетыCRM.ОписаниеТиповСодержитИерархическийТип(ДоступноеПоле.ТипЗначения) Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ ТолькоГруппировкиКолонок Тогда
		Для каждого Стр Из ПоляНастроек.ПолучитьЭлементы() Цикл
			...
			Для каждого ОписаниеРеквизита Из Стр.Реквизиты Цикл
				...
				//EXTCODE Шумилин Сергей 22.02.2017 {{ --->
				Если Стр.ДополнительныеПараметры.Свойство("ВыбранныеПоля") Тогда				
					Поле = ТекущийЭлементСтруктуры.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
					Поле.Поле = Новый ПолеКомпоновкиДанных("");
					Поле.Использование = Использование;
					Поле.ТипГруппировки = Стр.ДополнительныеПараметры.ТипГруппировки;		
					Для каждого ЭлементМассива Из Стр.ДополнительныеПараметры.ВыбранныеПоля Цикл
						ВыбранноеПоле 		= ТекущийЭлементСтруктуры.Выбор.Элементы.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
						ВыбранноеПоле.Поле  = Новый ПолеКомпоновкиДанных(Строка(ЭлементМассива));
					КонецЦикла;
				//EXTCODE Шумилин Сергей 22.02.2017 <--- }} 
				ИначеЕсли Стр.ПолучитьЭлементы().Количество()=0 Тогда
					Поле = ТекущийЭлементСтруктуры.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
...
Функция ПроверитьСовместимостьДиаграммы(ИмяПоляПроверки = "")
	...
	Если ЗначениеЗаполнено(ДиаграммаТочки) Тогда
		ПолеТочка = ОтчетыCRM.ПолеСхемы(Схема, ДиаграммаТочки);
		...
		Если ЗначениеЗаполнено(ДиаграммаСерии) Тогда
			ПолеСерия = ОтчетыCRM.ПолеСхемы(Схема, ДиаграммаСерии);
...
Процедура ДобавитьГруппировкуСравнения(СтруктураКолонок)
	...
	Если ТипЗнч(ПолеСравнения)=Тип("ПеречислениеСсылка.Периодичность") Тогда
		...
		СхемаКомпоновкиДанных = ПолучитьИзВременногоХранилища(НастройкиОтчета.АдресСхемы);
		Поле = ОтчетыCRM.ПолеСхемы(СхемаКомпоновкиДанных, ИмяПоля);
	...
	Для каждого ЭлементОтбора Из Настройки.Отбор.Элементы Цикл
		...
		Иначе
			...
			Если НЕ ТипЗнч(ЭлементОтбора.ЛевоеЗначение)=Тип("ПолеКомпоновкиДанных") Тогда
				Продолжить;
			КонецЕсли;
			//EXTCODE Шумилин Сергей 16.03.2017 {{ ---> Проверяем на наличие двух фильтров для случая диапазона по числу или дате
			//ДобавитьОписаниеНастройки(ПоляНастроек.ПолучитьЭлементы(), ТипФильтр(), Строка(ЭлементОтбора.ЛевоеЗначение), ЭлементОтбора.ПредставлениеПользовательскойНастройки, ЭлементОтбора);
			Стр = ДобавитьОписаниеНастройки(ПоляНастроек.ПолучитьЭлементы(), ТипФильтр(), Строка(ЭлементОтбора.ЛевоеЗначение), ЭлементОтбора.ПредставлениеПользовательскойНастройки, ЭлементОтбора);
			Если Стр <> Неопределено
					И Стр.ДополнительныеПараметры.Свойство("ЕстьДублирующееЗначение") Тогда
				ОбработатьВтороеЗначениеДляОтбораДиапазона(Стр, ЭлементОтбора);
			КонецЕсли;
			//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
		КонецЕсли; 
	КонецЦикла;
...
КонецПроцедуры
//EXTCODE Шумилин Сергей 28.02.2017 <--- }} 
...
Процедура ОпределитьЭлементыИРеквизитыФильтра(Стр, ЭлементОтбора = Неопределено, ДоступноеПоле, РучноеДобавление)
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Дата")) Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.БольшеИлиРавно 
			//EXTCODE Шумилин Сергей 14.04.2017 {{ --->
			//И Стр.Реквизиты[Идентификатор].ДатаНачала<Значение 
			//EXTCODE Шумилин Сергей 14.04.2017 <--- }} 
			Тогда
			Стр.Реквизиты[Идентификатор].ДатаНачала = Значение;
		ИначеЕсли ЭлементОтбора.ВидСравнения=ВидСравненияКомпоновкиДанных.МеньшеИлиРавно 
			//EXTCODE Шумилин Сергей 14.04.2017 {{ --->
			//И Стр.Реквизиты[Идентификатор].ДатаОкончания>Значение 
			//EXTCODE Шумилин Сергей 14.04.2017 <--- }} 
			Тогда
			Стр.Реквизиты[Идентификатор].ДатаОкончания = Значение;
		КонецЕсли; 
	...
	ИначеЕсли ТипЗначения.СодержитТип(Тип("Число")) Тогда
		...
		Если НЕ Стр.Реквизиты.Свойство(Идентификатор+"От") Тогда
			...
			Стр.Реквизиты.Вставить(Идентификатор+"От", ?(СтрЗаканчиваетсяНа(Стр.Заголовок,"От"),?(Значение=Неопределено, 0, Значение),0));
			Если СтрЗаканчиваетсяНа(Стр.Заголовок,"От") Тогда Стр.Заголовок = СтрЗаменить(Лев(Стр.Заголовок, СтрДлина(Стр.Заголовок)-2), "_", " ") КонецЕсли;
			//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
		КонецЕсли; 
		Если НЕ Стр.Реквизиты.Свойство(Идентификатор+"До") Тогда
			//EXTCODE Шумилин Сергей 16.03.2017 {{ --->
			//Стр.Реквизиты.Вставить(Идентификатор+"До", 0);
			Стр.Реквизиты.Вставить(Идентификатор+"До", ?(СтрЗаканчиваетсяНа(Стр.Заголовок,"До"),?(Значение=Неопределено, 0, Значение),0));		
			Если СтрЗаканчиваетсяНа(Стр.Заголовок,"До") Тогда Стр.Заголовок = СтрЗаменить(Лев(Стр.Заголовок, СтрДлина(Стр.Заголовок)-2), "_", " ") КонецЕсли;
			//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
		КонецЕсли;
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗначения.Типы().Получить(0)) Тогда
		...
		КоличествоЭлементов = ОтчетыCRM.ОпределитьКоличествоЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
		Если (КоличествоЭлементов=0 ИЛИ КоличествоЭлементов=1) И НЕ РучноеДобавление Тогда
			Возврат;
		КонецЕсли;
		Если КоличествоЭлементов<3 Тогда
			Значения = ОтчетыCRM.ПолучитьСписокЭлементов(ТипЗначения.Типы().Получить(0), ПараметрыВыбора);
			СтрокаЗаголовка = "";
...
Процедура УстановитьЗначениеФильтра(Поле, Значение, ВидСравнения = Неопределено, ОбновлятьВПанели = Истина, ЗаменитьЗначение = Ложь)
	...
	ИначеЕсли ТипЗнч(Значение)=Тип("СтандартныйПериод") Тогда
		...
		//EXTCODE Шумилин Сергей 24.03.2017 {{ --->
		НужноСоздаватьЭлементыОтбораДатаНачала 		= Истина;
		НужноСоздаватьЭлементыОтбораДатаОкончания 	= Истина;
		//EXTCODE Шумилин Сергей 24.03.2017 <--- }} 
		Для каждого ЭлементОтбор Из НайденныеЭлементы Цикл
			Если ЗначениеЗаполнено(ДатаНачала) И (ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.Больше ИЛИ ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.БольшеИлиРавно) Тогда
				...
				ЗначениеОт = Неопределено;
				//EXTCODE Шумилин Сергей 24.03.2017 {{ --->
				НужноСоздаватьЭлементыОтбораДатаНачала = Ложь;
				//EXTCODE Шумилин Сергей 24.03.2017 <--- }} 
				Продолжить;
			КонецЕсли; 
			Если ЗначениеЗаполнено(ДатаОкончания) И (ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.Меньше ИЛИ ЭлементОтбор.ВидСравнения=ВидСравненияКомпоновкиДанных.МеньшеИлиРавно) Тогда
				...
				ЗначениеОт = Неопределено;
				//EXTCODE Шумилин Сергей 24.03.2017 {{ --->
				НужноСоздаватьЭлементыОтбораДатаОкончания = Ложь;
				//EXTCODE Шумилин Сергей 24.03.2017 <--- }}				
				Продолжить;
			КонецЕсли;
		...
		ИмяОтметки = НовоеИмяОтметкиФильтра();
		//EXTCODE Шумилин Сергей 24.03.2017 {{ --->
		Если НужноСоздаватьЭлементыОтбораДатаНачала И
		//EXTCODE Шумилин Сергей 24.03.2017 <--- }} 
			ЗначениеЗаполнено(ДатаНачала) Тогда
			ЭлементОтбор = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбор.ЛевоеЗначение = ПолеКД;
			ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
		...
		КонецЕсли; 
		//EXTCODE Шумилин Сергей 24.03.2017 {{ --->
		Если НужноСоздаватьЭлементыОтбораДатаОкончания И
		//EXTCODE Шумилин Сергей 24.03.2017 <--- }} 
			ЗначениеЗаполнено(ДатаОкончания) Тогда
			ЭлементОтбор = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбор.ЛевоеЗначение = ПолеКД;
			ЭлементОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
...
//EXTCODE Шумилин Сергей 28.02.2017 {{ --->
&НаСервере
Процедура ОбработатьВтороеЗначениеДляОтбораДиапазона(Стр, ЭлементОтбора)

	ЧистыйЗаголовок = Лев(ЭлементОтбора.ПредставлениеПользовательскойНастройки, СтрДлина(ЭлементОтбора.ПредставлениеПользовательскойНастройки) - 2);
	
	Если СтрЗаканчиваетсяНа(ЭлементОтбора.ПредставлениеПользовательскойНастройки, "До")
		И Стр.Заголовок = ЧистыйЗаголовок Тогда
		Для каждого ЭлементСтруктуры Из Стр.Реквизиты Цикл
			Если СтрЗаканчиваетсяНа(ЭлементСтруктуры.Ключ, "До")
					И Не ЗначениеЗаполнено(ЭлементСтруктуры.Значение) Тогда
				Стр.Реквизиты[ЭлементСтруктуры.Ключ] = ЭлементОтбора.ПравоеЗначение;	
			КонецЕсли;	
		КонецЦикла;
	ИначеЕсли СтрЗаканчиваетсяНа(ЭлементОтбора.ПредставлениеПользовательскойНастройки, "От")
		И Стр.Заголовок = ЧистыйЗаголовок Тогда
		Для каждого ЭлементСтруктуры Из Стр.Реквизиты Цикл
			Если СтрЗаканчиваетсяНа(ЭлементСтруктуры.Ключ, "От")
					И Не ЗначениеЗаполнено(ЭлементСтруктуры.Значение) Тогда
				Стр.Реквизиты[ЭлементСтруктуры.Ключ] = ЭлементОтбора.ПравоеЗначение;	
			КонецЕсли;	
		КонецЦикла;		
...
&НаСервере
Процедура СоздатьПредопределенныйОтбор(ЭлементОтбора, ЗаголовокГруппировки = Неопределено)
...
Процедура ЗаполнитьТаблицуНаложенныхФильтров()
	...
	Для каждого Стр Из НаложенныеФильтры Цикл
		...
		Стр.Заголовок = ОтчетыCRMКлиентСервер.РучнаяКорректировкаНаименованияДоступныхПолейСКД(ДоступноеПоле.Заголовок);
		//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
		...
		ИначеЕсли ТипЗнч(Стр.Значение)=Тип("Булево") ИЛИ ТипЗнч(Стр.Значение)=Тип("Дата") Тогда
			ПолеСхемы = ОтчетыCRM.ПолеСхемы(СхемаКД, Стр.Поле);
...
Процедура ОтключитьВсеОтборы(Команда)
	...
	Для каждого Элемент Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		Если ТипЗнч(Элемент) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			Если НЕ Элемент.Представление = "Свободен (в пуле)" Тогда
				Для каждого _Элемент Из Элемент.Элементы Цикл
					Если НЕ (_Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Проект") 
							ИЛИ _Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПроектВидНедвижимости")
							ИЛИ _Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВПуле"))  Тогда
						_Элемент.Использование = Ложь;
					КонецЕсли;				
				КонецЦикла;
			КонецЕсли;
		Иначе
			Если НЕ (Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Проект") 
					ИЛИ Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ПроектВидНедвижимости")
					ИЛИ Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВПуле") 
					ИЛИ (Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Статус") И Элемент.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыОбъектовНедвижимости.Свободен"))) Тогда
				Элемент.Использование = Ложь;
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;	
...
Функция ДобавитьОписаниеНастройки(ЭлементыДобавления, Тип, ИмяПоля, Представление = "", ЭлементСхемы = Неопределено)
	...
	Если НЕ СуществующаяСтрока=Неопределено Тогда
		...
		//EXTCODE Шумилин Сергей 16.03.2017 {{ ---> Для определения дублей
		СуществующаяСтрока.ДополнительныеПараметры.Вставить("ЕстьДублирующееЗначение");
		//EXTCODE Шумилин Сергей 16.03.2017 <--- }} 
...
//EXTCODE Шумилин Сергей 15.03.2017 {{ --->
//Процедура обработки выбора пункта меню дополнительной расшифровки для отчета
//&НаКлиенте
//Процедура ПоказатьВыборДействияЗавершение(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры)
//	
//КонецПроцедуры
//EXTCODE Шумилин Сергей 15.03.2017 <--- }} 
...
//<>, Басаргин (10.11.2017) {
Функция ПолучитьВариантПоУмолчанию()
	
	КлючТекущегоВарианта = "ПодборОН";
	ПредставлениеВарианта = "Подбор ОН";
	...
	|	И ВариантыОтчетов.Наименование ПОДОБНО &ПредставлениеВарианта
	|	И НЕ ВариантыОтчетов.ПометкаУдаления";
	
	Запрос.УстановитьПараметр("ПредставлениеВарианта", ПредставлениеВарианта + "%");
...
//EXTCODE Шумилин Сергей 17.03.2017 {{ --->
&НаКлиенте
Процедура Подобрать(Команда)
	Если РезультатПодбора.Количество() > 0 Тогда
		//Если ЭтаФорма.ВладелецФормы.Листинг.Количество() > 0 Тогда
		//	ПоказатьВопрос(Новый ОписаниеОповещения("ПодобратьЗавершение", ЭтотОбъект), "Текущие данные в листинге будут заменены данными из подбора! Продолжить?", РежимДиалогаВопрос.ДаНет);		
		//Иначе
			Закрыть(ПолучитьМассивОНиОтборы());
		//КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПодобратьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Функция ПолучитьМассивОНиОтборы()

	СтруктураВозврата = Новый Структура;
	
	МассивОН = Новый Массив;
	Для Каждого СтрокаКоллекции Из РезультатПодбора Цикл
		МассивОН.Добавить(СтрокаКоллекции.ОбъектНедвижимости);	
	КонецЦикла;
	СтруктураВозврата.Вставить("МассивОН", 			МассивОН);
	СтруктураВозврата.Вставить("НаложенныеОтборы", 	Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы);
...
&НаКлиенте
Процедура УдалитьИзПодбора(Команда)
	
	ТекущаяСтрокаТаблицы = Элементы.РезультатПодбора.ТекущиеДанные;
	
	Если ТекущаяСтрокаТаблицы <> Неопределено Тогда
		РезультатПодбора.Удалить(ТекущаяСтрокаТаблицы);	
	КонецЕсли;
...
&НаСервере
Функция ПечатьОтчетаНаСервере(Знач ОтчетТабличныйДокумент)
	...
	Уровень = ПолучитьУровеньГруппировок(ОтчетОбъект.КомпоновщикНастроек.Настройки);
	ЭлементыОтбора = ОтчетОбъект.КомпоновщикНастроек.Настройки.Отбор.Элементы;
	Проект = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Проект");
	
	Если ТипЗнч(Проект) = Тип("СписокЗначений") И Проект.Количество() Тогда
		Проект = Проект[0].Значение;
	КонецЕсли;
	
	ТекущаяДата = ТекущаяДата();	
	Проект_ОС = ПолучитьОСПоПроекту(Проект);
	
	Если НЕ ЗначениеЗаполнено(Проект_ОС) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не заполнен объект строительства. Обратитесь к администратору");
		Возврат Неопределено;
	КонецЕсли;
		
	Если НЕ ЗначениеЗаполнено(Проект) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указан проект печати");
		Возврат Неопределено;
	КонецЕсли;	
	
	Если ТабДок.ВысотаТаблицы > 2 Тогда		
		Столбец = 1;
		Для й = 2 По ТабДок.ШиринаТаблицы Цикл
			Если Найти(ТабДок.Область(1, й).Текст, "№") > 0  ИЛИ Найти(ТабДок.Область(1, й).Текст, "Номер кв.") > 0 Тогда
				Столбец = й;
				Прервать;
			КонецЕсли;
		КонецЦикла;	
		ЦветВыделения = Новый Цвет(227, 255, 228);
		Для к = 1 По ТабДок.ВысотаТаблицы Цикл
			Если ТабДок.Область(к, Столбец, к, Столбец).ЦветФона = ЦветВыделения И НЕ ТабДок.Область(к, Столбец + 1, к, Столбец + 1).ЦветФона = Новый Цвет(0, 75, 136) Тогда
				ТабДок.Область(к, Столбец, к, Столбец).ЦветФона = ТабДок.Область(к, Столбец + 1, к, Столбец + 1).ЦветФона;
			КонецЕсли;	
		КонецЦикла;
		Если СтрНайти(Вариант.КлючВарианта,"ЗУ") Тогда
			Для к = 4 По ТабДок.ВысотаТаблицы Цикл
				Для й = 1 По ТабДок.ШиринаТаблицы Цикл
					//ТабДок.Область(к, й, к, й).ЦветФона =  Новый Цвет(255, 255, 255); 	
				КонецЦикла;		
			КонецЦикла;	
		КонецЕсли;
	КонецЕсли;	
	...
	ТабДок.ПолеСнизу = 20;
	ТабДок.ОриентацияСтраницы = ?(СтрНайти(Вариант.КлючВарианта,"ЗУ"), ОриентацияСтраницы.Портрет, ОриентацияСтраницы.Ландшафт);
	ТабДок.АвтоМасштаб = Истина;	
	ТабДок.РазмерКолонтитулаСнизу = 50;
	ТабДок.РазмерКолонтитулаСверху = 20;
	ТабДок.ПовторятьПриПечатиСтроки = ТабДок.Область(1, , Уровень, );		
	...
	ТабДок.ВерхнийКолонтитул.ТекстСлева = "Дата проведения расчета: " + Формат(ТекущаяДата, "ДФ='dd.MM.yyyy ""г.""'");
	ТабДок.ВерхнийКолонтитул.Выводить = Истина;
	ТабДок.ВерхнийКолонтитул.Шрифт = Новый Шрифт(, 12);
	
	ТабДок.НижнийКолонтитул.НачальнаяСтраница = 1;
	ТабДок.НижнийКолонтитул.ВертикальноеПоложение = ВертикальноеПоложение.Центр;
	//ТабДок.НижнийКолонтитул.ТекстВЦентре = Строка(Проект_ОС);
	
	Если НЕ СтрНайти(Вариант.КлючВарианта,"ЗУ") Тогда
		ТабДок.НижнийКолонтитул.ТекстСлева = ?(Прав(Строка(Проект_ОС), 1) = ".", Строка(Проект_ОС), Строка(Проект_ОС) + ".") + " Получить более подробную информацию, а также ознакомиться с условиями предоставления максимальной скидки можно на промо-сайте объекта, по телефону +7(495) 745-55-33 или у консультантов в офисе продаж." 
			+ Символы.ПС + "Цены действительны на дату формирования каталога предложений!";
	Иначе
		ТабДок.НижнийКолонтитул.ТекстВЦентре = ?(Прав(Строка(Проект_ОС), 1) = ".", Строка(Проект_ОС), Строка(Проект_ОС) + ".") + " Получить более подробную информацию можно на промо-сайте объекта, по телефону +7(495) 745-55-33 или у консультантов в офисе продаж." 
			+ Символы.ПС + "Цены действительны на дату формирования каталога предложений!";
	КонецЕсли;		
			
	ТабДок.НижнийКолонтитул.Выводить = Истина;
	ТабДок.НижнийКолонтитул.Шрифт = Новый Шрифт(, ?(НЕ СтрНайти(Вариант.КлючВарианта,"ЗУ"), 9, 6));
				
	КартинкаПроекта = ПолучитьКартинкуПроекта(Проект);	
	
	ТитульныйЛист = Новый ТабличныйДокумент;
	ТитульныйЛист.АвтоМасштаб = Истина;
	ТитульныйЛист.ЭкземпляровНаСтранице = 1;
	ТитульныйЛист.ПолеСверху = 10;
	ТитульныйЛист.ПолеСнизу = 10;
	ТитульныйЛист.ПолеСлева = 15;
	ТитульныйЛист.ПолеСправа = 10;
	ТитульныйЛист.ОриентацияСтраницы = ?(СтрНайти(Вариант.КлючВарианта,"ЗУ"), ОриентацияСтраницы.Портрет, ОриентацияСтраницы.Ландшафт);	
		
	МакетТитульныйЛист = ?(СтрНайти(Вариант.КлючВарианта,"ЗУ"), УправлениеПечатью.МакетПечатнойФормы("Отчет.Квартирограмма.ТитульныйЛистЗУ"), УправлениеПечатью.МакетПечатнойФормы("Отчет.Квартирограмма.ТитульныйЛист"));
	Шапка = МакетТитульныйЛист.ПолучитьОбласть("Шапка");
	ДатаПроведенияРасчета = МакетТитульныйЛист.ПолучитьОбласть("ДатаПроведенияРасчета");
	ПропускСтроки = МакетТитульныйЛист.ПолучитьОбласть("ПропускСтроки");
	КаталогПредложений = МакетТитульныйЛист.ПолучитьОбласть("КаталогПредложений");
	ЗаполнениеПараметров = МакетТитульныйЛист.ПолучитьОбласть("ЗаполнениеПараметров");  
	Подвал = МакетТитульныйЛист.ПолучитьОбласть("Подвал");

	ПараметрыПечати = Новый Структура();
	Попытка
		ПолучитьДанныеОбАкциях(Проект, Проект_ОС, ТекущаяДата, ПараметрыПечати);		
	Исключение
		ПараметрыПечати = Неопределено;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("При получении списка доступных акций произошла ошибка. Обратитесь к администратору");
	КонецПопытки;	
	
	Шапка.Рисунки.Логотип.Картинка = ?(ЗначениеЗаполнено(КартинкаПроекта), КартинкаПроекта, Новый Картинка);
	
	Если НЕ СтрНайти(Вариант.КлючВарианта,"ЗУ") Тогда
		Картинка_Субсидии = Шапка.Рисунки.Субсидии.Картинка;
		Картинка_ВоеннаяИпотека = Шапка.Рисунки.ВоеннаяИпотека.Картинка;
		Картинка_МатКапитал = Шапка.Рисунки.МатКапитал.Картинка;
		Картинка_ИпотекаБезПВ = Шапка.Рисунки.ИпотекаБезПВ.Картинка;
		
		ДопОпции = Новый Соответствие;
		ДопОпции.Вставить("ДопОпция1", Новый Структура("Название, Картинка", "", Новый Картинка)); 
		ДопОпции.Вставить("ДопОпция2", Новый Структура("Название, Картинка", "", Новый Картинка)); 
		ДопОпции.Вставить("ДопОпция3", Новый Структура("Название, Картинка", "", Новый Картинка));  
		ДопОпции.Вставить("ДопОпция4", Новый Структура("Название, Картинка", "", Новый Картинка));  
		
		Если НЕ ПараметрыПечати = Неопределено Тогда		  
			Если ЗначениеЗаполнено(ПараметрыПечати.Субсидии) Тогда
				Шапка.Параметры.ДопОпция1 = ПараметрыПечати.Субсидии;
				ДопОпции["ДопОпция1"].Название = Шапка.Параметры.ДопОпция1;
				ДопОпции["ДопОпция1"].Картинка = Картинка_Субсидии;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ПараметрыПечати.ВоеннаяИпотека) Тогда
				Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция1) Тогда
					Шапка.Параметры.ДопОпция2 = ПараметрыПечати.ВоеннаяИпотека;
					ДопОпции["ДопОпция2"].Название = Шапка.Параметры.ДопОпция2;
					ДопОпции["ДопОпция2"].Картинка = Картинка_ВоеннаяИпотека;	
				Иначе
					Шапка.Параметры.ДопОпция1 = ПараметрыПечати.ВоеннаяИпотека;
					ДопОпции["ДопОпция1"].Название = Шапка.Параметры.ДопОпция1;
					ДопОпции["ДопОпция1"].Картинка = Картинка_ВоеннаяИпотека;				
				КонецЕсли;
			КонецЕсли;

			Если ЗначениеЗаполнено(ПараметрыПечати.МатКапитал) Тогда
				Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция1) Тогда
					Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция2) Тогда
						Шапка.Параметры.ДопОпция3 = ПараметрыПечати.МатКапитал;
						ДопОпции["ДопОпция3"].Название = Шапка.Параметры.ДопОпция3;
						ДопОпции["ДопОпция3"].Картинка = Картинка_МатКапитал;
					Иначе
						Шапка.Параметры.ДопОпция2 = ПараметрыПечати.МатКапитал;
						ДопОпции["ДопОпция2"].Название = Шапка.Параметры.ДопОпция2;
						ДопОпции["ДопОпция2"].Картинка = Картинка_МатКапитал;				
					КонецЕсли;
				Иначе
					Шапка.Параметры.ДопОпция1 = ПараметрыПечати.МатКапитал;
					ДопОпции["ДопОпция1"].Название = Шапка.Параметры.ДопОпция1;
					ДопОпции["ДопОпция1"].Картинка = Картинка_МатКапитал;				
				КонецЕсли;
			КонецЕсли;	
			
			Если ЗначениеЗаполнено(ПараметрыПечати.ИпотекаБезПВ) Тогда
				Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция1) Тогда
					Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция2) Тогда			
						Если ЗначениеЗаполнено(Шапка.Параметры.ДопОпция3) Тогда
							Шапка.Параметры.ДопОпция4 = ПараметрыПечати.ИпотекаБезПВ;
							ДопОпции["ДопОпция4"].Название = Шапка.Параметры.ДопОпция4;
							ДопОпции["ДопОпция4"].Картинка = Картинка_ИпотекаБезПВ;	
						Иначе
							Шапка.Параметры.ДопОпция3 = ПараметрыПечати.ИпотекаБезПВ;
							ДопОпции["ДопОпция3"].Название = Шапка.Параметры.ДопОпция3;
							ДопОпции["ДопОпция3"].Картинка = Картинка_ИпотекаБезПВ;						
						КонецЕсли;
					Иначе
						Шапка.Параметры.ДопОпция2 = ПараметрыПечати.ИпотекаБезПВ;
						ДопОпции["ДопОпция2"].Название = Шапка.Параметры.ДопОпция2;
						ДопОпции["ДопОпция2"].Картинка = Картинка_ИпотекаБезПВ;					
					КонецЕсли;
				Иначе
					Шапка.Параметры.ДопОпция1 = ПараметрыПечати.ИпотекаБезПВ;
					ДопОпции["ДопОпция1"].Название = Шапка.Параметры.ДопОпция1;
					ДопОпции["ДопОпция1"].Картинка = Картинка_ИпотекаБезПВ;			
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Для каждого ДопОпция Из ДопОпции Цикл
			Если ДопОпция.Ключ = "ДопОпция1" Тогда
				Шапка.Рисунки.Субсидии.Картинка = ДопОпция.Значение.Картинка;
			КонецЕсли;
			Если ДопОпция.Ключ = "ДопОпция2" Тогда
				Шапка.Рисунки.ВоеннаяИпотека.Картинка = ДопОпция.Значение.Картинка;
			КонецЕсли;
			Если ДопОпция.Ключ = "ДопОпция3" Тогда
				Шапка.Рисунки.МатКапитал.Картинка = ДопОпция.Значение.Картинка;
			КонецЕсли;
			Если ДопОпция.Ключ = "ДопОпция4" Тогда
				Шапка.Рисунки.ИпотекаБезПВ.Картинка = ДопОпция.Значение.Картинка;
			КонецЕсли;		
		КонецЦикла;	
	КонецЕсли;

	ТитульныйЛист.Вывести(Шапка);
	
	ДатаПроведенияРасчета.Параметры.ДатаПроведенияРасчета = Формат(ТекущаяДата, "ДФ='dd.MM.yyyy ""г.""'");	
	ТитульныйЛист.Вывести(ДатаПроведенияРасчета);
	
	ТитульныйЛист.Вывести(ПропускСтроки);
	ТитульныйЛист.Вывести(ПропускСтроки);
	ТитульныйЛист.Вывести(ПропускСтроки);
	
	КаталогПредложений.Параметры.НазваниеПроекта = Строка(Проект_ОС);
	
	ТитульныйЛист.Вывести(КаталогПредложений);
	
	//Заполнение параметров
	СтоимостьДо = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "СтоимостьСУчетомСкидки", ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);		
	ЗаполнениеПараметров.Параметры.СтоимостьДо = ?(ЗначениеЗаполнено(СтоимостьДо), СтоимостьДо, "");	
	
	ПлощадьОт = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Площадь", ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	ПлощадьДо = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Площадь", ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);	
	ЗаполнениеПараметров.Параметры.Метраж = Строка(?(ЗначениеЗаполнено(ПлощадьОт), ПлощадьОт, "0")) + " - " + Строка(?(ЗначениеЗаполнено(ПлощадьДо), ПлощадьДо, " и более"));
	
	Если НЕ СтрНайти(Вариант.КлючВарианта,"ЗУ") Тогда 
		СписокКомнат = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "КоличествоКомнат");
		СтрКомнаты = "";
		Если НЕ СписокКомнат = Неопределено Тогда
			Для Каждого ЭлК Из СписокКомнат Цикл
				СтрКомнаты = СтрКомнаты + ?(ЗначениеЗаполнено(СтрКомнаты), "; ", "") + Строка(ЭлК);
			КонецЦикла;
		КонецЕсли;
		ЗаполнениеПараметров.Параметры.Комнаты = СтрКомнаты; 
		
		СрокСдачиДо = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "СрокСдачи", ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
		ЗаполнениеПараметров.Параметры.СрокСдачиДо = ?(НЕ СрокСдачиДо = Неопределено, Формат(СрокСдачиДо, "ДФ=dd.MM.yyyy"), "");
		
		ЭтажОт = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Этаж", ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
		ЭтажДо = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Этаж", ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
		НеПервыйЭтаж = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Этаж", ВидСравненияКомпоновкиДанных.НеРавно, , "НЕ первый этаж");
		ПервыйЭтаж = ?(НеПервыйЭтаж = Неопределено, Ложь, Истина);
		Если НЕ ЗначениеЗаполнено(ПлощадьОт) И НЕ ЗначениеЗаполнено(ПлощадьДо) и ПервыйЭтаж Тогда
			ЗаполнениеПараметров.Параметры.Этаж = "Все";
		Иначе
			ЗаполнениеПараметров.Параметры.Этаж = Строка(?(ЗначениеЗаполнено(ЭтажОт), ?(ПервыйЭтаж, ЭтажОт, ?(ЭтажОт = 1, "1", ЭтажОт)), ?(ПервыйЭтаж, "1", "0"))) + " - " + Строка(?(ЗначениеЗаполнено(ЭтажДо), ЭтажДо, " и выше"));
		КонецЕсли;
		ЗаполнениеПараметров.Параметры.ПервыйЭтаж = ПервыйЭтаж;
		
		НеПоследнийЭтаж = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Этаж", ВидСравненияКомпоновкиДанных.НеРавно, , "НЕ последний этаж");
		ПоследнийЭтаж = ?(НеПоследнийЭтаж = Неопределено, Ложь, Истина);
		ЗаполнениеПараметров.Параметры.ПоследнийЭтаж = ПоследнийЭтаж;
		
		Фазы = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Фаза");
		СтрФазы = "";
		Если НЕ Фазы = Неопределено Тогда
			Для Каждого ЭлК Из Фазы Цикл
				СтрФазы = СтрФазы + ?(ЗначениеЗаполнено(СтрФазы), "; ", "") + Строка(ЭлК);
			КонецЦикла;
		КонецЕсли;	
		ЗаполнениеПараметров.Параметры.Фаза = СтрФазы;	
		
		Панель = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "ВидМатериала", ВидСравненияКомпоновкиДанных.Равно, Перечисления.ВидыМатериалов.Панель);
		Монолит = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "ВидМатериала", ВидСравненияКомпоновкиДанных.Равно, Перечисления.ВидыМатериалов.Монолит);	
		ТипыДомов = "";
		ТипыДомов = ТипыДомов + ?(ЗначениеЗаполнено(Панель), "Панель", "");
		ТипыДомов = ТипыДомов + ?(ЗначениеЗаполнено(ТипыДомов) И ЗначениеЗаполнено(Монолит), "; Монолит", "");	
		ЗаполнениеПараметров.Параметры.ТипДома = ТипыДомов;
		
		СОтделкой = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Отделка", ВидСравненияКомпоновкиДанных.Равно, Истина);
		БезОтделки = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Отделка", ВидСравненияКомпоновкиДанных.Равно, Ложь);
		
		СОтделкой = ?(ЗначениеЗаполнено(СОтделкой), Истина, Ложь);
		БезОтделки = ?(ЗначениеЗаполнено(БезОтделки), Истина, Ложь);	
		//ЗаполнениеПараметров.Параметры.СОтделкой = ?(ЗначениеЗаполнено(СОтделкой), Истина, Ложь);
		//ЗаполнениеПараметров.Параметры.БезОтделки = ?(ЗначениеЗаполнено(БезОтделки), Истина, Ложь);			
		
		ЗаполнениеПараметров.Параметры.Отделка = ?(СОтделкой, "С отделкой", "") + ?(БезОтделки, ?(СОтделкой, "; без отделки", "Без отделки"), "");
		
		Корпуса = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "Корпус");
		СтрКорпуса = "";
		Если НЕ Корпуса = Неопределено Тогда
			Для Каждого ЭлК Из Корпуса Цикл
				СтрКорпуса = СтрКорпуса + ?(ЗначениеЗаполнено(СтрКорпуса), "; ", "") + Строка(ЭлК);
			КонецЦикла;
		КонецЕсли;	
		ЗаполнениеПараметров.Параметры.Корпус = СтрКорпуса;	
		
		ОднаСекция = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "КоличествоСекций", ВидСравненияКомпоновкиДанных.Равно, 1);
		//ЗаполнениеПараметров.Параметры.ОднаСекция = ?(ЗначениеЗаполнено(ОднаСекция), Истина, "");
		
		НесколькоСекций = ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, "КоличествоСекций", ВидСравненияКомпоновкиДанных.Больше, 1);
		//ЗаполнениеПараметров.Параметры.НесколькоСекций = ?(ЗначениеЗаполнено(НесколькоСекций), Истина, "");
		
		ОднаСекция = ?(ЗначениеЗаполнено(ОднаСекция), Истина, Ложь);
		НесколькоСекций = ?(ЗначениеЗаполнено(НесколькоСекций), Истина, Ложь);
		ЗаполнениеПараметров.Параметры.Секции = ?(ОднаСекция, "Одна секция", "") + ?(НесколькоСекций, ?(ОднаСекция, "; несколько секций", "Несколько секций"), "");	
		
		//СпГруппировка = "";
		//ПолучитьПредставлениеГруппировок(ОтчетОбъект.КомпоновщикНастроек.Настройки, СпГруппировка);
		//СпГруппировка = СокрЛП(СпГруппировка);
		//ЗаполнениеПараметров.Параметры.Группировка = ?(НЕ ПустаяСтрока(СпГруппировка), ?(Сред(СпГруппировка, СтрДлина(СпГруппировка), 1) = ";", Сред(СпГруппировка, 1, СтрДлина(СпГруппировка) - 1), СпГруппировка), "");
	КонецЕсли;
	
	Если НЕ ПараметрыПечати = Неопределено Тогда
		Для каждого МК Из ПараметрыПечати.МаркетинговыеКампании Цикл
			Попытка
				ЗаполнениеПараметров.Параметры["Акция" + МК.НомерАкции] = МК.МаркетинговаяКампания;
				ЗаполнениеПараметров.Параметры["ОписаниеАкции" + МК.НомерАкции] = МК.Описание;
			Исключение
				Продолжить;
			КонецПопытки;
		КонецЦикла;	
	...
	ТитульныйЛист.Вывести(ЗаполнениеПараметров);
	...
	//Пока ТитульныйЛист.ПроверитьВывод(МассивОбластейДляПроверки) Цикл
	//	ТитульныйЛист.Вывести(ПропускСтроки);
	//	КоличествоПропусков = КоличествоПропусков + 1;
	//	МассивОбластейДляПроверки.Очистить();
	//	Для й = 1 По КоличествоПропусков Цикл
	//		МассивОбластейДляПроверки.Добавить(ПропускСтроки);
	//	КонецЦикла;
	//	МассивОбластейДляПроверки.Добавить(Подвал);
	//КонецЦикла;
	
	ТитульныйЛист.Вывести(Подвал);
		
	Пакет = Новый ПакетОтображаемыхДокументов;
	Пакет.КоличествоЭкземпляров = 1;
	//Если СтрНайти(Вариант.КлючВарианта,"ЗУ") Тогда
		ЭлементПакета1 = Пакет.Состав.Добавить();
		ЭлементПакета1.Данные = ПоместитьВоВременноеХранилище(ТитульныйЛист, УникальныйИдентификатор);
	//КонецЕсли;
	ЭлементПакета2 = Пакет.Состав.Добавить();
	ЭлементПакета2.Данные = ПоместитьВоВременноеХранилище(ТабДок, УникальныйИдентификатор);
...
&НаСервере
Процедура ПолучитьДанныеОбАкциях(Проект, ОбъектСтроительства, ТекущаяДата, ПараметрыПечати)
	
	ПроксиСервер = Неопределено;
	ПроектБазыДомостроителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Проект, "ПроектБазыДомостроителя");
	CRMСервер.ПодключитьсяКСервису(ПроксиСервер, ПроектБазыДомостроителя, "PrintReportData");
	
	Если НЕ ПроксиСервер = Неопределено Тогда
		//ОбъектСтроительства = ПолучитьОСПоПроекту(Проект);
		Если ЗначениеЗаполнено(ОбъектСтроительства) Тогда
			СписокКодовМК = ?(ПроектБазыДомостроителя, Константы.КодыМК_ПечатьКвартирограммы_ДС.Получить(), Константы.КодыМК_ПечатьКвартирограммы.Получить());
			ДанныеТаблицыЗначений = ПроксиСервер.GetData(СериализаторXDTO.XMLСтрока(ОбъектСтроительства), ТекущаяДата, ?(ЗначениеЗаполнено(СписокКодовМК), СписокКодовМК, "000000078;000000079;000000080;000000081")); 		
			СтруктураТаблиц = СериализаторXDTO.XMLЗначение(Тип("ХранилищеЗначения"), ДанныеТаблицыЗначений).Получить();
			Если ЗначениеЗаполнено(СтруктураТаблиц.ТекстОшибки) Тогда
				Сообщить(СтруктураТаблиц.ТекстОшибки);
				Возврат;
			КонецЕсли;
			ПараметрыПечати = ПреобразоватьОтветВебСервиса(СтруктураТаблиц);
		КонецЕсли;
	КонецЕсли;		
...
&НаСервере
Функция ПолучитьОСПоПроекту(Проект)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫБОР
	|		КОГДА ОбъектыСтроительства.Проект.ВидНедвижимости = &ЖилаяНедвижимость
	|			ТОГДА ОбъектыСтроительства.Ссылка.Родитель
	|		ИНАЧЕ ОбъектыСтроительства.Ссылка
	|	КОНЕЦ КАК Ссылка
	|ИЗ
	|	Справочник.ОбъектыСтроительства КАК ОбъектыСтроительства
	|ГДЕ
	|	ОбъектыСтроительства.Проект = &Проект
	|	И НЕ ОбъектыСтроительства.Наименование ПОДОБНО &Служебный";
	
	Запрос.УстановитьПараметр("Проект", Проект);
	Запрос.УстановитьПараметр("ЖилаяНедвижимость", Перечисления.ВидыОбъектовНедвижимости.ЖилаяНедвижимость);
	Запрос.УстановитьПараметр("Служебный", "%" + "служебный" + "%");
	
	РезультатЗапроса = Запрос.Выполнить();	
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();	
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ТаблицаИерархии = ПолучитьВсеУровниИерархии();
		Для каждого Группа ИЗ ТаблицаИерархии Цикл
			Если ВыборкаДетальныеЗаписи.Ссылка = Группа.ЗначениеЭлемента Тогда //Если ВыборкаДетальныеЗаписи.Ссылка.ПринадлежитЭлементу(Группа.Уровень1) ИЛИ ВыборкаДетальныеЗаписи.Ссылка = Группа.Уровень1 Тогда
				Возврат Группа.Уровень1;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьВсеУровниИерархии(ИмяСпр = "ОбъектыСтроительства")
	...
	|ВЫБРАТЬ
	|   ИмяСпр.Ссылка КАК ЗначениеЭлемента
	|   , ИмяСпр.Ссылка КАК Уровень0";
	Для Сч=1 По МаксУровень Цикл
		Запрос.Текст = Запрос.Текст + "
		|   , ЗНАЧЕНИЕ(Справочник." + ИмяСпр + ".ПустаяСсылка) КАК Уровень"+Сч;
	КонецЦикла;
	Запрос.Текст = Запрос.Текст + "
	|ИЗ
	|   Справочник." + ИмяСпр + " КАК ИмяСпр
	|ГДЕ
	|   ИмяСпр.Родитель = ЗНАЧЕНИЕ(Справочник." + ИмяСпр + ".ПустаяСсылка)";
	...
	Для ТекущийУровень = 1 По МаксУровень Цикл
		...
		КонецЦикла;
		Запрос.Текст = Запрос.Текст + "
		|   , ИмяСпр.Ссылка КАК Уровень"+ТекущийУровень;
		...
		Для Сч2=1 по ТекущийУровень Цикл
			Запрос.Текст = Запрос.Текст + ".Родитель";
		КонецЦикла;
		Запрос.Текст = Запрос.Текст + " = ЗНАЧЕНИЕ(Справочник." + ИмяСпр + ".ПустаяСсылка)";
	КонецЦикла;
...
&НаСервере
Функция ПолучитьЗначениеЭлементаОтбора(ЭлементыОтбора, Поле, ВидСравнения = Неопределено, ПравоеЗначение = Неопределено, Представление = Неопределено)
	
	Для каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
		Если ЭлементОтбора.Использование И ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(Поле) 
			И ?(ВидСравнения = Неопределено, Истина, ЭлементОтбора.ВидСравнения = ВидСравнения)
			И ?(ПравоеЗначение = Неопределено, Истина, ЭлементОтбора.ПравоеЗначение = ПравоеЗначение)
			И ?(Представление = Неопределено, Истина, ЭлементОтбора.Представление = Представление) Тогда
...
&НаСервере
Процедура ПолучитьПредставлениеГруппировок(Настройки, СпГруппировка = "")
	
	Если Настройки.Структура.Количество() Тогда
		Для каждого Элемент Из Настройки.Структура Цикл
			Если Элемент.ПоляГруппировки.Элементы.Количество() И (НЕ СтрНайти(Строка(Элемент.ПоляГруппировки.Элементы[0].Поле), СпГруппировка) ИЛИ ПустаяСтрока(СпГруппировка))  Тогда
				СпГруппировка = СпГруппировка + Элемент.ПоляГруппировки.Элементы[0].Поле + "; " + Символы.ПС;	
			КонецЕсли;
		КонецЦикла;		
		ПолучитьПредставлениеГруппировок(Настройки.Структура[0], СпГруппировка);
	Иначе
		Возврат;
	КонецЕсли;	
...
&НаСервере
Функция ПолучитьУровеньГруппировок(Настройки, Уровень = 0)
	
	Для каждого Элемент Из Настройки.Структура Цикл
		Уровень = ПолучитьУровеньГруппировок(Элемент, Уровень + 1);
	КонецЦикла;
...
&НаСервере
Функция ПолучитьОбластьШапки(пМакет, пПроект)
	
	ОбластьШапка = пМакет.ПолучитьОбласть("ШапкаПоУмолчанию");
	Попытка	
		ОбластьШапка.Рисунки.Макет.Картинка = Новый Картинка(пПроект.Макет.Получить());	
	Исключение
	КонецПопытки;
...
Функция ПолучитьКартинкуПроекта(Проект)
	...
	Попытка 
		Возврат Новый Картинка(Проект.Макет.Получить());
	Исключение
	КонецПопытки;
...
&НаСервере
Функция ПреобразоватьОтветВебСервиса(Таблицы)
		
	СтруктураПараметрыОтбора = Новый Структура();
	СтруктураПараметрыОтбора.Вставить("МаркетинговыеКампании", Новый Массив);
	
	СтруктураПараметрыОтбора.Вставить("Субсидии", "");
	СтруктураПараметрыОтбора.Вставить("ВоеннаяИпотека", "");
	СтруктураПараметрыОтбора.Вставить("МатКапитал", "");
	СтруктураПараметрыОтбора.Вставить("ИпотекаБезПВ", "");	
		
	ТЗДляВозврата_Опции = Новый ТаблицаЗначений;
	ТЗДляВозврата_Опции.Колонки.Добавить("Субсидии");	
	ТЗДляВозврата_Опции.Колонки.Добавить("ВоеннаяИпотека");
	ТЗДляВозврата_Опции.Колонки.Добавить("МатКапитал");	
	ТЗДляВозврата_Опции.Колонки.Добавить("ИпотекаБезПВ");	

	ТЗДляВозврата_МК = Новый ТаблицаЗначений;
	ТЗДляВозврата_МК.Колонки.Добавить("МаркетинговаяКампания");
	ТЗДляВозврата_МК.Колонки.Добавить("Описание");
	
	Таблицы.ТаблицаМК.Сортировать("Описание УБЫВ");
	НомерАкции = 1;
	Для Каждого СтрокаТЗ из Таблицы.ТаблицаМК Цикл		
		НоваяСтрока = ТЗДляВозврата_МК.Добавить();		
		НоваяСтрока.МаркетинговаяКампания = СериализаторXDTO.XMLЗначение(Тип("Строка"), СтрокаТЗ.МаркетинговаяКампания); //СериализаторXDTO.XMLЗначение(Тип("СправочникСсылка.МаркетинговыеКомпании"), СтрокаТЗ.МаркетинговаяКомпания);
		НоваяСтрока.Описание = СериализаторXDTO.XMLЗначение(Тип("Строка"), СтрокаТЗ.Описание);		
		СтруктураМК = Новый Структура();
		СтруктураМК.Вставить("МаркетинговаяКампания");
		СтруктураМК.Вставить("Описание");
		ЗаполнитьЗначенияСвойств(СтруктураМК, НоваяСтрока);
		СтруктураМК.Вставить("НомерАкции", НомерАкции);
		СтруктураПараметрыОтбора.МаркетинговыеКампании.Добавить(СтруктураМК);
		НомерАкции = НомерАкции + 1;
	КонецЦикла;

	Для Каждого СтрокаТЗ из Таблицы.ТаблицаОпций Цикл		
		НоваяСтрока = ТЗДляВозврата_Опции.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЗ);
		Если НЕ НоваяСтрока.Субсидии = NULL И НоваяСтрока.Субсидии Тогда 
			СтруктураПараметрыОтбора.Субсидии = "Субсидии";
		КонецЕсли;		
		Если НЕ НоваяСтрока.Субсидии = NULL И НоваяСтрока.ВоеннаяИпотека Тогда 
			СтруктураПараметрыОтбора.ВоеннаяИпотека = "Военная ипотека";
		КонецЕсли;
		Если НЕ НоваяСтрока.Субсидии = NULL И НоваяСтрока.МатКапитал Тогда 
			СтруктураПараметрыОтбора.МатКапитал = "Материнский капитал";
		КонецЕсли;
		Если НЕ НоваяСтрока.Субсидии = NULL И НоваяСтрока.ИпотекаБезПВ Тогда 
			СтруктураПараметрыОтбора.ИпотекаБезПВ = "Ипотека без перв. взноса";
		КонецЕсли;	
		//ЗаполнитьЗначенияСвойств(СтруктураПараметрыОтбора, НоваяСтрока);
	КонецЦикла;
	
	Возврат СтруктураПараметрыОтбора;
...
Процедура ПечатьОтчета(Команда)
	...
	Пакет = ПечатьОтчетаНаСервере(ОтчетТабличныйДокумент); 
...
&НаСервере
Функция ПечатьТаблицыНаСервере()
	...
	//Шапка
	СтрокаТаблицы = ОтчетТабличныйДокумент.ПолучитьОбласть(1, 1, КоличествоУровнейГруппировокСтрок, ОтчетТабличныйДокумент.ШиринаТаблицы);
	ТД.Вывести(СтрокаТаблицы);
	
	НомерКолонки = 0;
	Для Счетчик = 1 По ОтчетТабличныйДокумент.ШиринаСтраницы Цикл
		Если СтрНайти(ОтчетТабличныйДокумент.Область(1, Счетчик, 1, Счетчик).Текст, "Номер кв.") <> 0 Тогда
	...
	//Тело
	Для Стр = КоличествоУровнейГруппировокСтрок + 1 По ОтчетТабличныйДокумент.ВысотаТаблицы Цикл
		Если СтрНайти(ОтчетТабличныйДокумент.Область(Стр, НомерКолонки, Стр, НомерКолонки).Текст, "Всего") = 0 Тогда
			СтрокаТаблицы = ОтчетТабличныйДокумент.ПолучитьОбласть(Стр, 1, Стр, ОтчетТабличныйДокумент.ШиринаТаблицы);
			ТД.Вывести(СтрокаТаблицы);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ПечатьТаблицы(Команда)
	
	ТД = ПечатьТаблицыНаСервере();
	
	Если ТД = Неопределено Тогда
		Сообщить("Отсутствует колонка ""Номер кв."", печать в таблицу не возможна!");
	Иначе
...
//++
&НаСервереБезКонтекста
Функция ВидимостьКнопкиПечатиТаблицы(Вариант)
	Возврат Вариант = Справочники.ВариантыОтчетов.НайтиПоНаименованию("Подбор ОН", Истина);
КонецФункции
//--
