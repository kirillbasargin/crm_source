...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоМенеджерФилиала_ОСС = (УправлениеДоступом.ЕстьРоль("МенеджерФилиала", , ПользователиКлиентСервер.АвторизованныйПользователь()) ИЛИ УправлениеДоступом.ЕстьРоль("МенеджерОСС", , ПользователиКлиентСервер.АвторизованныйПользователь()))
								И НЕ УправлениеДоступом.ЕстьРоль("ПолныеПрава", , ПользователиКлиентСервер.АвторизованныйПользователь()); 
	Если ЭтоМенеджерФилиала_ОСС Тогда
		Параметры.КлючВарианта = "ШахматкаДанныеБТИ";
	КонецЕсли;	
	//<840014> }
	...
	//EXTCODE Шумилин Сергей 24.04.2017 {{ --->
	УстановитьДанныеПоВыбранномуОбъектуНедвижимости(Параметры.ОбъектНедвижимостиВыделение);
	...
	Элементы.ОтображениеПанелиНастроек.Заголовок = ?(Элементы.ОтображениеПанелиНастроек.Пометка, "Показать панель", "Скрыть панель");
	Элементы.ОтключитьОтборПоПодъезду.Видимость = ЗначениеЗаполнено(Параметры.Подъезд);
	Элементы.ОтключитьОтборПоПодъезду.Заголовок = ?(ЗначениеЗаполнено(Параметры.Подъезд), "Сбросить фильтр: подъезд " + Параметры.Подъезд, "Подъезд"); //?(ЗначениеЗаполнено(Параметры.Подъезд), "Подъезд " + Параметры.Подъезд, "Подъезд");
...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	//<845969>, Басаргин (19.07.2018) {
	Если ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") И ФормаПараметры.Отбор.Количество() > 0 Тогда
		ИзменитьСКД();	
...
Процедура УстановитьПредзаданныеОтборы(ПользНастройки)
	...
	Если не ФормаПараметры.Свойство("Отбор") ИЛИ НЕ ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") ИЛИ ФормаПараметры.Отбор.Количество()=0 Тогда 
		Возврат;
	КонецЕсли;
	
	рТипЭО=Тип("ЭлементОтбораКомпоновкиДанных");
	
	Для каждого киз Из ФормаПараметры.Отбор Цикл
		...
		рНужныйЭО=Неопределено; 
		Для каждого элнастр Из ПользНастройки.Элементы Цикл
			Если ТипЗнч(элнастр)=Тип("ОтборКомпоновкиДанных") и рНужныйОтбор=Неопределено Тогда 
				рНужныйОтбор=элнастр; 
		...
		Если рНужныйОтбор<>Неопределено Тогда 
			рНужныйЭОизОтбора=Неопределено;
			Для каждого элотб Из рНужныйОтбор.Элементы Цикл
				Если элотб.ЛевоеЗначение=рПоле Тогда рНужныйЭОизОтбора=элотб;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		...
		ИначеЕсли рНужныйОтбор=Неопределено и рНужныйЭО<>Неопределено Тогда 
		...
		Для каждого элотб Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл 
...
Процедура ИзменитьСКД()
	...
	Для каждого эл Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		...
		Если ПустаяСтрока(эл.ИдентификаторПользовательскойНастройки) Тогда
			эл.ИдентификаторПользовательскойНастройки="7e4a7f97-d8b8-4596-ba71-bb7547a63f86"; 
			эл.ПредставлениеПользовательскойНастройки="Объект строительства";
		КонецЕсли;		
	КонецЦикла;
	
	комп=Новый КомпоновщикНастроекКомпоновкиДанных;
	Если ЗначениеЗаполнено(КлючТекущегоВарианта) Тогда
		Для каждого Настройка Из рОбъект.СхемаКомпоновкиДанных.ВариантыНастроек Цикл
			Если Настройка.Имя = КлючТекущегоВарианта Тогда
				ВариантНастроек = Настройка.Настройки;
				Прервать;	
			КонецЕсли;
		КонецЦикла;
	Иначе
		ВариантНастроек = рОбъект.СхемаКомпоновкиДанных.НастройкиПоУмолчанию;		
	КонецЕсли;
			
	комп.ЗагрузитьНастройки(ВариантНастроек); 
	...
	ПараметрСКД = рОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Подъезд");
	ПараметрСКД.Значение = Параметры.Подъезд;
	
	Если ЗначениеЗаполнено(Параметры.Подъезд) Тогда
		ПараметрСКД.Использование = Истина;		
	Иначе
		ПараметрСКД.Использование = Ложь;
	КонецЕсли;
	
	ПараметрСКД = рОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ОбъектНедвижимостиВыделение");
	ПараметрСКД.Значение = Параметры.ОбъектНедвижимостиВыделение;
	Если ЗначениеЗаполнено(Параметры.ОбъектНедвижимостиВыделение) Тогда
		ПараметрСКД.Использование = Истина;
	Иначе
		ПараметрСКД.Использование = Ложь;		
	КонецЕсли;	
		
	АдресСхемы = ПоместитьВоВременноеХранилище(рОбъект.СхемаКомпоновкиДанных, УникальныйИдентификатор);
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	//EXTCODE Шумилин Сергей 06.03.2017 {{ --->
	...
	СтандартнаяОбработка 		= Ложь;
	СтруктураДанныхПоКвартире 	= ПолучитьНавигационнуюСсылкуНаСервере(Расшифровка, ОтчетДанныеРасшифровки, УникальныйИдентификатор);
	Если СтруктураДанныхПоКвартире <> Неопределено Тогда
		АдресКартинки	  	= СтруктураДанныхПоКвартире.АдресКартинки;
		АдресПланировкиСекции = СтруктураДанныхПоКвартире.АдресПланировкиСекции;
		АдресКартинкиСхемы	= СтруктураДанныхПоКвартире.АдресКартинкиСхемы;
		СтоимостьКвартиры 	= СтруктураДанныхПоКвартире.Стоимость;
		Фаза				= СтруктураДанныхПоКвартире.Фаза; 
		Корпус              = СтруктураДанныхПоКвартире.Корпус;
		Подъезд             = СтруктураДанныхПоКвартире.Подъезд;
		Этаж                = СтруктураДанныхПоКвартире.Этаж;
		НомерНаПлощадке     = СтруктураДанныхПоКвартире.НомерНаПлощадке;
		ТипОтделки          = СтруктураДанныхПоКвартире.ТипОтделки;
		ЦенаЗаМетр          = СтруктураДанныхПоКвартире.ЦенаЗаМетр;
		ОбъектНедвижимости	= СтруктураДанныхПоКвартире.ОбъектНедвижимости;
		МаксимальнаяСкидка	= СтруктураДанныхПоКвартире.МаксимальнаяСкидка;
		Статус				= СтруктураДанныхПоКвартире.Статус;
		Собственник			= СтруктураДанныхПоКвартире.Собственник;
		СтатусПула 			= СтруктураДанныхПоКвартире.СтатусПула;
		РасшифровкаСтатуса  = СтруктураДанныхПоКвартире.РасшифровкаСтатуса;
		
		//<840014>, Басаргин (03.07.2018) {
		СтроительныйНомер	= СтруктураДанныхПоКвартире.СтроительныйНомер;
		ПлощадьПроектная	= СтруктураДанныхПоКвартире.ПлощадьПроектная;			
		//<840014> }
		
	Иначе
		АдресКартинки 		= Неопределено;
		АдресПланировкиСекции = Неопределено;
		АдресКартинкиСхемы	= Неопределено;
		СтоимостьКвартиры 	= 0;
		Фаза				= Неопределено; 
		Корпус              = Неопределено;
		...
		Этаж                = 0;
		НомерНаПлощадке     = 0;
		ТипОтделки          = Неопределено;
		ЦенаЗаМетр          = 0;
		ОбъектНедвижимости	= Неопределено;
		МаксимальнаяСкидка	= 0;
		Статус 				= Неопределено;
		Собственник  		= Неопределено;	
		СтатусПула 			= Неопределено;
		РасшифровкаСтатуса  = Неопределено;
		
		//<840014>, Басаргин (03.07.2018) {
		СтроительныйНомер	= Неопределено;
		ПлощадьПроектная	= Неопределено;			
		//<840014> }
				
	КонецЕсли;	
	//EXTCODE Шумилин Сергей 06.03.2017 <--- }}
		
	//ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СрокиБронированияОбъектовНедвижимости, "ОбъектНедвижимости", ОбъектНедвижимости, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный);
	СрокиБронированияОбъектовНедвижимости.Параметры.УстановитьЗначениеПараметра("ОбъектНедвижимостиВыделение", ОбъектНедвижимости);	
	
	Элементы.Группа_ТолькоДляВыводаВПул.Видимость = СтрНайти(ОтчетНаименованиеТекущегоВарианта, "Вывод в пул");
...
&НаКлиенте
Процедура ЦветаСтатусов(Команда)
	ОткрытьФорму("Отчет.Шахматка.Форма.ФормаЦветаСтатусов");
КонецПроцедуры
...
Процедура Подобрать(Команда)
	...
	Если РезультатПодбора.Количество() > 0 Тогда	
		Если ТипЗнч(ВладелецФормы) = Тип("УправляемаяФорма") И СтрНайти(ВладелецФормы.ИмяФормы, "Запрос") Тогда	
			Попытка
				ВладелецФормы.ОписаниеОповещенияОЗакрытии(ПолучитьМассивОНиОтборы(), Неопределено);
			Исключение
			КонецПопытки;
		ИначеЕсли НЕ ВладелецФормы = Неопределено И ТипЗнч(ВладелецФормы.ВладелецФормы) = Тип("УправляемаяФорма") И СтрНайти(ВладелецФормы.ВладелецФормы.ИмяФормы, "Запрос") Тогда
			Попытка
				ВладелецФормы.ВладелецФормы.ОписаниеОповещенияОЗакрытии(ПолучитьМассивОНиОтборы(), Неопределено);
			Исключение
			КонецПопытки;
		Иначе
			//Оповестить("ПодборОбъектов", РезультатПодбора, "Шахматка");
		КонецЕсли;
		Закрыть();
	КонецЕсли;		
...
&НаКлиенте
Процедура ЗаявкаНаВывод(Команда)
	
	Если ЗначениеЗаполнено(ОбъектНедвижимости) Тогда
		ОткрытьФорму("Документ.ЗаявкаНаВыводОНВПродажу.ФормаОбъекта", Новый Структура("ОбъектНедвижимости", ОбъектНедвижимости));	
	КонецЕсли;
...
&НаКлиенте
Процедура ОтображениеПанелиНастроек(Команда)
	
	Элементы.ОтображениеПанелиНастроек.Пометка = НЕ Элементы.ОтображениеПанелиНастроек.Пометка;
	Элементы.ОтображениеПанелиНастроек.Заголовок = ?(Элементы.ОтображениеПанелиНастроек.Пометка, "Показать панель", "Скрыть панель");
	Элементы.ПанельНастроек.Видимость = НЕ Элементы.ОтображениеПанелиНастроек.Пометка;
	
КонецПроцедуры

&НаКлиенте
Процедура ОтключитьОтборПоПодъезду(Команда)
	
	ОтключитьОтборПоПодъездуНаСервере();
	ОчиститьСообщения();
	Сформировать();
...
&НаКлиенте
Процедура ДобавитьВЛистинг(Команда)
	ДобавитьВЛистингНаСервере();
КонецПроцедуры
...
Процедура ПослеФормированияНаКлиенте()
	...
	КонецЕсли;
	//EXTCODE Шумилин Сергей 07.03.2017 {{ --->
	ОбработатьЗаголовок(ОтчетТабличныйДокумент);
	//EXTCODE Шумилин Сергей 07.03.2017 <--- }} 	
	ОтчетыКлиентПереопределяемый.ПослеФормирования(ЭтотОбъект, ОтчетСформирован);
	
	ПодключитьОбработчикОжидания("ПередвинутьКурсорНаЭтаж", 0.1, Истина);
	Элементы.Группа_ТолькоДляВыводаВПул.Видимость = СтрНайти(ОтчетНаименованиеТекущегоВарианта, "Вывод в пул");
	
КонецПроцедуры

//EXTCODE Шумилин Сергей 23.10.2017 {{ --->
&НаКлиенте
...
//EXTCODE Шумилин Сергей 07.03.2017 {{ --->
&НаКлиенте
Процедура ОбработатьЗаголовок(ТабДок)
...
Функция ОбъединятьЯчейки(ТабДок, индСтр, индКол)
    ...
    ИначеЕсли
        ...
        Лев(Ячейка.Текст, 3) = Лев(ЯчейкаСлед.Текст, 3)
...
//EXTCODE Шумилин Сергей 06.03.2017 {{ --->
&НаСервереБезКонтекста
Функция ПолучитьНавигационнуюСсылкуНаСервере(Расшифровка, ДанныеРасшифровки, ИдентификаторФормы)
	...
	НайденноеПоле = ПоляРасшифровки.Найти("ОбъектНедвижимости");
	...
	Иначе
		...
		Попытка
			АдресКартинки = ПрисоединенныеФайлы.ПолучитьДанныеФайла(НайденноеПоле.Значение.ПланировкаКвартиры, ИдентификаторФормы).СсылкаНаДвоичныеДанныеФайла;
		Исключение
			АдресКартинки = Неопределено;
		КонецПопытки;
		
		Попытка
			АдресПланировкиСекции = ПрисоединенныеФайлы.ПолучитьДанныеФайла(НайденноеПоле.Значение.ПланировкаСекции, ИдентификаторФормы).СсылкаНаДвоичныеДанныеФайла;
		Исключение
			АдресПланировкиСекции = Неопределено;
		КонецПопытки;
		
		Попытка
			АдресКартинкиСхемы = ПрисоединенныеФайлы.ПолучитьДанныеФайла(НайденноеПоле.Значение.СхемаРасположения, ИдентификаторФормы).СсылкаНаДвоичныеДанныеФайла;
		Исключение
			АдресКартинкиСхемы = Неопределено;
		КонецПопытки;		
		
		Результат = Новый Структура("АдресКартинки, АдресПланировкиСекции, АдресКартинкиСхемы,Стоимость,Фаза,Корпус,Подъезд,НомерНаПлощадке,ЦенаЗаМетр,ТипОтделки,Этаж,ОбъектНедвижимости,МаксимальнаяСкидка,СЗ_Скидки, Статус, Собственник, СтатусПула, РасшифровкаСтатуса, СтроительныйНомер, ПлощадьПроектная",
										АдресКартинки, АдресПланировкиСекции, АдресКартинкиСхемы,0,Неопределено,0,Неопределено,0,0,Неопределено,0,Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено);
		
		НайденноеПоле = ПоляРасшифровки.Найти("Стоимость");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Стоимость = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("ОбъектНедвижимости");                                                                                     
		Если НайденноеПоле <> Неопределено Тогда
			Результат.ОбъектНедвижимости = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("Фаза");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Фаза = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("Корпус");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Корпус = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("Подъезд");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Подъезд = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("НомерНаПлощадке");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.НомерНаПлощадке = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("ЦенаЗаМетр");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.ЦенаЗаМетр = НайденноеПоле.Значение;
		КонецЕсли;		
				
		НайденноеПоле = ПоляРасшифровки.Найти("ТипОтделки");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.ТипОтделки = НайденноеПоле.Значение;
		КонецЕсли;
		
		НайденноеПоле = ПоляРасшифровки.Найти("Этаж");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Этаж = НайденноеПоле.Значение;
		КонецЕсли;	
		
		НайденноеПоле = ПоляРасшифровки.Найти("МаксимальнаяСкидка");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.МаксимальнаяСкидка = НайденноеПоле.Значение;
						
		КонецЕсли;		
		
		НайденноеПоле = ПоляРасшифровки.Найти("Статус");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Статус = НайденноеПоле.Значение;
		КонецЕсли;	
		
		НайденноеПоле = ПоляРасшифровки.Найти("Собственник");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.Собственник = НайденноеПоле.Значение;
		КонецЕсли;	
		
		НайденноеПоле = ПоляРасшифровки.Найти("СтатусПула");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.СтатусПула = НайденноеПоле.Значение;
		КонецЕсли;	
		
		НайденноеПоле = ПоляРасшифровки.Найти("РасшифровкаСтатуса");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.РасшифровкаСтатуса = НайденноеПоле.Значение;
		КонецЕсли;		
		
		//<840014>, Басаргин (03.07.2018) {
		НайденноеПоле = ПоляРасшифровки.Найти("СтроительныйНомер");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.СтроительныйНомер = НайденноеПоле.Значение;
		КонецЕсли;		
		
		НайденноеПоле = ПоляРасшифровки.Найти("ПлощадьПроектная");
		Если НайденноеПоле <> Неопределено Тогда
			Результат.ПлощадьПроектная = НайденноеПоле.Значение;
		КонецЕсли;		
		//<840014> }	
...
//EXTCODE Шумилин Сергей 24.04.2017 {{ ---> Аналогичная выше процедуре - только выполняется заполнение по запросу
&НаСервере
Процедура УстановитьДанныеПоВыбранномуОбъектуНедвижимости(ПараметрОбъектНедвижимости)

	Если ЗначениеЗаполнено(ПараметрОбъектНедвижимости) Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ОбъектыНедвижимости.ПланировкаКвартиры КАК ПланировкаКвартиры,
		|	ОбъектыНедвижимости.ПланировкаСекции КАК ПланировкаСекции,
		|	ОбъектыНедвижимости.СхемаРасположения КАК СхемаРасположения,
		...
		|	Квартирограмма.Статус КАК Статус,
		|	ОбъектыНедвижимости.Собственник КАК Собственник,
		...
		|		ПО Квартирограмма.ОбъектНедвижимости = ОбъектыНедвижимости.Ссылка
		...
		Запрос.УстановитьПараметр("Ссылка", ПараметрОбъектНедвижимости);
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Попытка
				АдресКартинки = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.ПланировкаКвартиры, УникальныйИдентификатор).СсылкаНаДвоичныеДанныеФайла;
			Исключение
				АдресКартинки = Неопределено;
			КонецПопытки;
			
			Попытка
				АдресПланировкиСекции = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.ПланировкаСекции, УникальныйИдентификатор).СсылкаНаДвоичныеДанныеФайла;
			Исключение
				АдресПланировкиСекции = Неопределено;
			КонецПопытки;			
			
			Попытка
				АдресКартинкиСхемы = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.СхемаРасположения, УникальныйИдентификатор).СсылкаНаДвоичныеДанныеФайла;
			Исключение
				АдресКартинкиСхемы = Неопределено;
			КонецПопытки;	
			
			СтоимостьКвартиры 	= Выборка.Стоимость;
			Фаза				= Выборка.Фаза; 
			Корпус              = Выборка.Корпус;
			Подъезд             = Выборка.Подъезд;
			Этаж                = Выборка.Этаж;
			НомерНаПлощадке     = Выборка.НомерНаПлощадке;
			ТипОтделки          = Выборка.ТипОтделки;
			ЦенаЗаМетр          = Выборка.ЦенаЗаМетр;
			ОбъектНедвижимости	= ПараметрОбъектНедвижимости;
			МаксимальнаяСкидка	= Выборка.МаксимальнаяСкидка;			
		    Статус 				= Выборка.Статус;
			Собственник 		= Выборка.Собственник;
			СтатусПула 			= Выборка.СтатусПула;
			РасшифровкаСтатуса	= Выборка.РасшифровкаСтатуса;
			
			//<840014>, Басаргин (03.07.2018) {
			СтроительныйНомер	= Выборка.СтроительныйНомер;
			ПлощадьПроектная	= Выборка.ПлощадьПроектная;			
			//<840014> }
			
			//ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СрокиБронированияОбъектовНедвижимости, "ОбъектНедвижимости", Параметры.ОбъектНедвижимостиВыделение, ВидСравненияКомпоновкиДанных.Равно, , Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Обычный);
			СрокиБронированияОбъектовНедвижимости.Параметры.УстановитьЗначениеПараметра("ОбъектНедвижимостиВыделение", Параметры.ОбъектНедвижимостиВыделение);
...
//EXTCODE Шумилин Сергей 21.04.2017 <--- }} 

&НаКлиенте
Процедура ПередвинутьКурсорНаЭтаж() Экспорт
	
	ЭтаФорма.ТекущийЭлемент = Элементы.ОтчетТабличныйДокумент;
	Если ЭтаФорма.ВводДоступен() И ЗначениеЗаполнено(Параметры.Подъезд) Тогда
		Попытка
			WSHShell = Новый COMОбъект("WScript.Shell");
			//WSHShell.SendKeys("{TAB 3}");	
			//WSHShell.SendKeys("{CLICK LEFT,1,1}");
			WSHShell.SendKeys("{DOWN " + ?(ЗначениеЗаполнено(Параметры.ЭтажПерехода) И ЗначениеЗаполнено(Параметры.ВсегоЭтажей), (Параметры.ВсегоЭтажей - Параметры.ЭтажПерехода) + 4, 0) + "}"); // ?( (Параметры.ВсегоЭтажей - Параметры.ЭтажПерехода) = 0, 0, (Параметры.ВсегоЭтажей - Параметры.ЭтажПерехода) + 4)
	        //WSHShell.SendKeys("{ENTER}");			 
		Исключение				
		КонецПопытки;
        ОтключитьОбработчикОжидания("ПередвинутьКурсорНаЭтаж");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ОтключитьОтборПоПодъездуНаСервере()
	
	Элементы.ОтключитьОтборПоПодъезду.Видимость = Ложь;
	Элементы.ОтключитьОтборПоПодъезду.Заголовок = "";
	
	Параметры.Подъезд = Неопределено;

	//<845969>, Басаргин (20.07.2018) {
	ИзменитьСКД();	
	Если ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") И ФормаПараметры.Отбор.Количество() > 0 Тогда
		УстановитьПредзаданныеОтборы(Отчет.КомпоновщикНастроек.ПользовательскиеНастройки);
	КонецЕсли;	
	...
	//ОтчетОбъект = РеквизитФормыВЗначение("Отчет");
	//ПараметрСКД = ОтчетОбъект.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Подъезд");
	//ПараметрСКД.Использование = Ложь;
...
Функция ПолучитьМассивОНиОтборы()
	...
	СтруктураВозврата.Вставить("НаложенныеОтборы", 	Неопределено); //Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы
...
&НаСервере
Процедура ДобавитьВЛистингНаСервере()
	
	Если ЗначениеЗаполнено(ОбъектНедвижимости) Тогда		
		Если ЭтаФорма.РезультатПодбора.НайтиСтроки(Новый Структура("ОбъектНедвижимости", ОбъектНедвижимости)).Количество() > 0 Тогда
			Возврат;			
		КонецЕсли;		
		НоваяСтрока = ЭтаФорма.РезультатПодбора.Добавить();
		НоваяСтрока.ОбъектНедвижимости 	= ОбъектНедвижимости;
		НоваяСтрока.ОбъектСтроительства = Строка(ОбъектНедвижимости.Владелец);		
	КонецЕсли;
